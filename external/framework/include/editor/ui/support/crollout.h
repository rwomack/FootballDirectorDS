...
//
// TimJ:
// 14/01/2002 04:48 : Changed CRolloutCtrlT<> to accept a child window
//                    handle rather than performing a create itself.
// 25/09/2007 18:32 : Removed win95 mouse wheel support and #ifdef it.
// Copyright (C) 2005-2007 Four Door Lemon Ltd. All Rights Reserved.

#ifndef EDITOR_UI_SUPPORT_CROLLOUT_H_INCLUDED
#define EDITOR_UI_SUPPORT_CROLLOUT_H_INCLUDED
...
// Overrideables
    ...
    // TimJ:
    {
      unsigned long activeBorderBGR       = 0x606060;
      unsigned long activeBackgroundBGR   = 0xEDEAE9;
      unsigned long checkedBorderBGR      = 0xB5ABA9;
      unsigned long checkedBackgroundBGR  = 0xCAC7C7;

      CRect rect = rcClient;
      CBrush  brush;
      brush.CreateSolidBrush(m_fFocus ? activeBorderBGR : checkedBorderBGR );
      dc.FrameRect(rect, brush);
      rect.DeflateRect(1,1,1,1);

      unsigned long backgroundBGR = m_fChecked ? checkedBackgroundBGR : activeBackgroundBGR;
      dc.FillSolidRect(rect, backgroundBGR);
    }
...
// Construction
  CRolloutCtrlImpl(HWND hwndChild) : m_Child(this, 1), m_fExpanded(1), m_fFocus(0), m_fPressed(0), m_fEnabled(0)
  {
    //TimJ: Added hwndChild
    m_Child.Attach(hwndChild);
  }

  // TimJ: Call to unsubclass the child window before it is destroyed using DestroyWindow()
  void DetachChild()
  {
    m_Child.ShowWindow( SW_HIDE );
    m_Child.SetWindowPos(NULL, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
    CWindow(m_Child.m_hWnd).ModifyStyle( WS_CHILD, WS_POPUP );
    ::SetParent( m_Child.m_hWnd, NULL );
    m_Child.UnsubclassWindow();
...
// Implementation
  void Init()
    ...
    //TimJ: m_Child.TChild::Create(m_hWnd, (LPARAM)NULL);
    ::CWindow(m_Child.m_hWnd).ModifyStyle( WS_POPUP, WS_CHILD );
    ::SetParent( m_Child.m_hWnd, m_hWnd );
    //
    ...
    // TimJ: Move the window to the top left of the parent
    m_Child.SetWindowPos(NULL, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);

		m_Child.ShowWindow(SW_SHOWNORMAL);
		m_Child.GetWindowRect(&m_rcExpanded);

    ////TimJ: limit the width to 200
    //if( m_rcExpanded.right-m_rcExpanded.left > 200 ) m_rcExpanded.right = m_rcExpanded.left+200;
...
public:
  ...
  //TimJ:  Added hwndChild
  CRolloutCtrl(HWND hwndChild) : CRolloutCtrlImpl<CRolloutCtrl, TChild>(hwndChild)
	{ }
};
...
//				m_nCtrlsOffsetY += nDeltaY;
	...
	LRESULT OnContextMenu(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		for(int i = 0; i < m_aRolloutCtrls.GetSize(); i++)
		{
			...
			TCHAR szName[256];
			::GetWindowText(m_aRolloutCtrls[i], szName, 255);
			if(::SendMessage(m_aRolloutCtrls[i], RCM_ISEXPANDED, 0, 0))
				uFlags |= MF_CHECKED;
			RECT rc;
			::GetWindowRect(m_aRolloutCtrls[i], &rc);
			ScreenToClient(&rc);
