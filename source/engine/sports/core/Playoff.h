...
public:
	...
	void					DoCreateFinishingOrder(CNumericArray<ClubID>& _ClubIDList);
	void					DoDraw();
	void					DoEndOfSeason();
	void					DoInitialiseFromResource(CBinaryReader& _Resource);
	CCup*					GetCup();
	byte					GetNumberPromoted()	const;
	void					GetWinners(CNumericArray<ClubID>& _ClubIDList, bool _AwardTrophy);
	byte					GetID()	const;
...
public:
	...
	CPlayOff*				DoFindFromId(const ushort _Value);
