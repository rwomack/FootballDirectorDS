...
void CTransferDeadline::DoAnnouncements(CActiveCountry& _ActiveCountry)
{
	...
	for (uint ListPos = 0; ListPos < MAX_DEADLINE_WINDOWS; ListPos++)
	{
		...
		{
			_ActiveCountry.AddToNewsLists(CurrentNewsItem);
			break;
		}
	}
}
...
{
	m_TransferDeadlineList = NULL;
	m_TransferDeadlineItemList = NULL;
}
...
CTransferDeadlineSystem::~CTransferDeadlineSystem()
{
	if (m_TransferDeadlineList != NULL)
	{
		delete []m_TransferDeadlineList;
	}
	if (m_TransferDeadlineItemList != NULL)
	{
...
void CTransferDeadlineSystem::DoInitialise()
{
	...
	m_TransferDeadlineList = FD_ENGINE_NEW CTransferDeadline[CExtArray::GetSize()];
	...
	m_TransferDeadlineItemList = FD_ENGINE_NEW CTransferDeadlineItem[m_HowManyItems];
