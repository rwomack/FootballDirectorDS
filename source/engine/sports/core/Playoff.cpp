...
void CPlayOff::DoDraw()
{
	...
	else
	{
		...
		switch(pLastCupRound->GetRoundType())
		{
			...
			case GROUP415372648:
				CCup::GetGroupMatchWinners(DrawClubs, pCurrentCupRound->GetNumberOfClubsInRound());
				break;
			default :
#endif
...
void CPlayOff::Read(CDataFile* _pDataFile)
{
	...
	_pDataFile->Read(&m_NumberPromoted, sizeof(m_NumberPromoted));
...
void CPlayOff::Write(CDataFile* _pDataFile)
{
	...
	_pDataFile->Write(&m_NumberPromoted, sizeof(m_NumberPromoted));
...
/*------------------------------------------------------------------------------
	Method:   	CPlayOff::DoCreateFinishingOrder
	Access:    	public 
	Parameter: 	CNumericArray<ClubID> & _ClubIDList
	Returns:   	void
	Qualifier: 	
	Purpose:
------------------------------------------------------------------------------*/
void CPlayOff::DoCreateFinishingOrder(CNumericArray<ClubID>& _ClubIDList)
{
...
/*------------------------------------------------------------------------------
	...
	Purpose:	Builds list of entrants in finishing order and returns winners
				Calls EndOfSeason
------------------------------------------------------------------------------*/
void CPlayOff::GetWinners(CNumericArray<ClubID>& _ClubIDList, bool _AwardTrophy)
{
    CNumericArray<ClubID> Losers;
    CCupRound* pLastCupRound = static_cast<CCupRound*> (CCup::GetFinalRound());
    EXT_ASSERT(pLastCupRound != NULL);
...
// 				NumGroups = FirstCupRound.GetNumberOfClubsInRound() / FirstCupRound.GetNumberInGroup();
...
CPlayOffList::CPlayOffList()
{
	...
	{
		m_pList = FD_ENGINE_NEW CPlayOff[CExtArray::GetSize()];
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
bool CPlayOffList::IsClubMatchToday(const CCalendar& _CurrentDate, const ClubID _ClubID, CMatchInfo& _Match)
{
...
/*------------------------------------------------------------------------------
	Method:   	CPlayOffList::DoFindFromId
	Access:    	public 
	...
	Purpose:
------------------------------------------------------------------------------*/
CPlayOff* CPlayOffList::DoFindFromId(const ushort _Value)
{
	CPlayOff* pPlayOff = m_pList;
	if (GetSize() > 0)
	{
