...
private:
	...
	struct
	{
		ePlayerPosition			m_Position:3;                     // Player's natural position
		uint					m_BestFoot:2;					  // best foot
...
public:
	...
	void					DoEnsureInRange();
  	byte          			DoGenerateRandomSkill(const float aSkill, const byte aMinSkill, const byte aRndSkill);
  	void          			DoCheckExperienceForLevelUp(float _CareerPathMultiplier);
  	void          			DoCheckExperienceForLevelDown(float _CareerPathMultiplier);
	...
	void					DoLowerFitness(const uint _Amount);
    int                     EnergyText();
	int						FitnessStrID(const int _Fitness) const;
	float          			GetEnergyEffect() const;
	int						GetFitnessStrID() const;
    byte					FindType();
    ...
    bool              		MaxValue(float Amount, const byte Age, CCountry& rCountry, const byte Temper);
	...
	ushort					DoGenerateRandomContractLength(const byte _Age);
	...
	void 					DoTrainStrength(const byte _Hours, const byte _Age);
  	void          			DoTrainSkills(float _Percentage, const byte _Age);
	void					TrainYouth(const byte Age, CClub& _Club);
	void					DoInjuredTraining();
	void					SetOverallSkill(const byte _Value);
