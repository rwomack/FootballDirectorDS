...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CChairman::DoNewSeason(CDivision& _Division, CClub &cClub)
{
	CIndexedIntArray<ClubID> TotalSkillList;
	for (int LoopCount = 0; LoopCount < _Division.GetNumberOfClubsInDivision(); LoopCount++)
	{
		CClub& theClub = WorldData().GetClubByID(_Division.GetClubs()[LoopCount]);
	...
	m_SeasonTargetPosition = TotalSkillList.IndexPosition(cClub.DoFindID());
	m_TargetPointsForMonth = 0xff;
	m_PointsThisMonth = 0;
