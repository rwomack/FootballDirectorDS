...
{
	EXT_ASSERT(m_BidClubID.id != NOCLUB);
	return WorldData().GetClubByID(m_BidClubID);
}
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
ClubID CClubTransferOffer::GetBidClubID()
{
	EXT_ASSERT(m_BidClubID.id != NOCLUB);
	return m_BidClubID;
}
...
{
	const uint uiAskingPrice = WorldData().GetTransferListManager().GetAskingPrice(GetPlayerID());
	if (GetAmountOffered() >= uiAskingPrice)
	{
		return true;
	}
	if (GetAmountOffered() > max(GetPlayer().DoCalculateMinimumTransferFee(rCountry), (uiAskingPrice / 100) * (75 + RandomNumber.IntLessThan(20))))
	{
		return true;
	}
  m_szReasonString.Format(IDS_BIDTOOLOW, strClubName, CNewsItem::PreprocessPlayerName(&GetPlayer()).CStr());
	return false;
}
...
{
  float FairPrice = GetPlayer().GetSellingPrice(_Country);
  if (GetAmountOffered() < (FairPrice * .8f) || \
    (IsBetween(static_cast<float>(GetAmountOffered()), FairPrice * .8f,  FairPrice * .84f) && RandomNumber.IntLessThan(100) > 5) || \
    (IsBetween(static_cast<float>(GetAmountOffered()), FairPrice * .85f, FairPrice * .89f) && RandomNumber.IntLessThan(100) > 10) || \
    (IsBetween(static_cast<float>(GetAmountOffered()), FairPrice * .90f, FairPrice * .94f) && RandomNumber.IntLessThan(100) > 30) || \
    (IsBetween(static_cast<float>(GetAmountOffered()), FairPrice * .95f, FairPrice) && RandomNumber.IntLessThan(100) > 60))
	{
...
{
	if (WorldData().GetTransferListManager().IsTransferListed(GetPlayerID()) == false)
	{
		// Always consider very large bids
		if (GetAmountOffered() < WorldData().GetTransferListManager().GetAskingPrice(GetPlayerID()) * 1.8)
		{
