...
{
	uint DatesToNegotiation = 1 + RandomNumber.IntLessThan(2);
	CCalendar CurrentDate;
	CurrentDate = WorldData().GetCurrentDate();

	for (uint LoopCount = 0; LoopCount < DatesToNegotiation; LoopCount++)
	{
	...
	CString str;
#ifdef NDS
    str.Format(IDS_PERSONALTERMS, CNewsItem::PreprocessPlayerName(&_rTheBid.GetPlayer()).CStr(), WorldData().GetForwardDateShortStr(DatesToNegotiation));
#else
...
{
#ifdef YOUTHTRAINING
    m_pYouthSquadTrainingDetails = FD_ENGINE_NEW CSquadTraining();
#endif
	m_pTacticsList = FD_ENGINE_NEW CTacticList();
    m_pPlayerSearch = FD_ENGINE_NEW CPlayerSearch();
    m_pNewsList = FD_ENGINE_NEW CNewsList();
    m_pTransferNegotiationList = FD_ENGINE_NEW CTransferNegotiationList();
}
...
void CUser::DoManageNewClub(CClub& _ClubToManage, const uint _Amount, const ushort _Years)
{
	...
	_ClubToManage.DoManagerJoinedClub(m_ManagerID);
...
void CUser::Write(CDataFile* _pDataFile)
{
    ...
    m_pTransferNegotiationList->Write(_pDataFile);
  	_pDataFile->Write(&m_Chairman, sizeof(CChairman));
#ifdef TROPHYHISTORY
...
#ifdef TROPHYHISTORY
	m_TrophyHistory.AddChampions(_Division.GetName(), GetClubID(), _Season, _Division.GetTrophyID(), false);
#endif
	CExtString str;
...
void CUser::Champions(CCup* _Cup, const CString &_Season)
{
	EXT_ASSERT(IsManagingAClub() == true);
#ifdef TROPHYHISTORY
	m_TrophyHistory.AddChampions(_Cup->GetName(), GetClubID(), _Season, _Cup->GetTrophyID(), true);
#endif
	CExtString str;
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CUser::WonCharityShield(const CString& _Title)
{
	EXT_ASSERT(IsManagingAClub() == true);
	CExtString str;
  str.Format(IDS_USERWONSHIELD, GetManager().GetName(), _Title);
	AddToNews(str);
}
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CUser::WonPlayoff(CPlayOff* pPlayOff, const CString& Season)
{
	EXT_ASSERT(IsManagingAClub() == true);
#ifdef TROPHYHISTORY
	m_TrophyHistory.AddChampions(pPlayOff->GetName(), GetClubID(), Season, pPlayOff->GetTrophyID(), true);
#endif
}
...
void CUser::RunnersUpPlayoff(CPlayOff* pPlayOff, const CString& Season)
{
	EXT_ASSERT(IsManagingAClub() == true);
#ifdef TROPHYHISTORY
	m_TrophyHistory.AddRunnersUp(pPlayOff->GetName(), GetClubID(), Season, pPlayOff->GetTrophyID(), true);
#endif
}
...
void CUser::RunnersUp(CDivision& _Division, const CString &Season)
{
	EXT_ASSERT(IsManagingAClub() == true);
#ifdef TROPHYHISTORY
	m_TrophyHistory.AddRunnersUp(_Division.GetName(), GetClubID(), Season, _Division.GetTrophyID(), false);
#endif
}
...
void CUser::RunnersUp(CCup* pCup, const CString &Season)
{
	EXT_ASSERT(IsManagingAClub() == true);
#ifdef TROPHYHISTORY
	m_TrophyHistory.AddRunnersUp(pCup->GetName(), GetClubID(), Season, pCup->GetTrophyID(), true);
#endif
}
...
void CUser::TrophyWinners(const CString& title, const byte Id, const CString& Season, bool IsCup)
{
	EXT_ASSERT(IsManagingAClub() == true);
#ifdef TROPHYHISTORY
	m_TrophyHistory.AddChampions(title, GetClubID(), Season, Id, IsCup);
#endif
}
...
void CUser::TrophyRunnersUp(const CString& title, const byte Id, const CString& Season, bool IsCup)
{
	EXT_ASSERT(IsManagingAClub() == true);
#ifdef TROPHYHISTORY
	m_TrophyHistory.AddRunnersUp(title, GetClubID(), Season, Id, IsCup);
#endif
}
...
{
	bool bRet = false;
	if (WorldData().IsFastTest() == false && (IsManagingAClub() == true) && GetClub().GetDivision().IsCloseSeasonToday(WorldData().GetCurrentDate().GetMatchDay()) == true)
	{
		bRet = true;
		CString str;
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CUser::AddToNews(const CExtString &_Message, bool _bUserRelated )
{
	EXT_ASSERT(IsManagingAClub() == true);
#ifdef NDS
	m_pNewsList->Add(FD_ENGINE_NEW CNewsItem(_Message, NEWSALL, WorldData().GetCurrentDate(), _bUserRelated));
	// Highlight news rollout icon
  	theApp.OnHighlightMenuIcon( HIGHLIGHT_NEWS );
#else
	m_pNewsList->Add(new CNewsItem(_Message, WorldData().GetCurrentDate(), _bUserRelated));
#endif
}
...
#ifdef NDS
			else if (GetClub().IsValidUserControlledTeam(Match.GetSubsSelect(), cInvalidityReason) == false)
#else
...
{
	return WorldData().GetPopularityList()[_PopularityID].IsUserGoodEnoughToManageClub((byte)GetManager().GetRatingController().GetRating());
}
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
CCountry& CUser::GetCountry()
{
	return WorldData().GetCountry(GetCountryID());
}
...
/*------------------------------------------------------------------------------
	Method:   	CUser::DoPreMatchReportFDL
	Access:    	public
	Parameter: 	lArray<CString> * _pStringList
	Returns:   	void
	Qualifier:
	Purpose:
------------------------------------------------------------------------------*/
void CUser::DoPreMatchReportFDL(lArray<CString> * _pStringList)
{
    CMatchInfo MatchDetails;
	CString szStr;
#ifdef TEST_MATCH_REPORT
  _pStringList->Add(IDS_WONALLGAMES, 1);
  _pStringList->Add(IDS_NOTWONAGAME, 1);
  _pStringList->AddString(IDS_WONAGAME);
  ...
  szStr.Format(IDS_CONCEDEDFEW, 13);
	_pStringList->Add(szStr, 1);
	WorldData().GetClub(44).DoPreMatchReport(_pStringList);
#else
	if (IsMatchToday(WorldData().GetCurrentDate(), MatchDetails) == true)
	{
		...
		if (OpponentClub.IsNonLeagueClub() == false && GetClub().DoCountLeagueGamesPlayedThisSeason() > 1)
		{
          ...
          szStr.Format(IDS_CONCEDEDFEW, OpponentClub.GetTableData().GetNumberGoalsConceded());
				...
				OpponentClub.DoPreMatchReportFDL(_pStringList);
			}
		}
	}
#endif
...
CUserList::CUserList(uint _HowMany)
{
	...
	{
        SetAt(n, FD_ENGINE_NEW CUser(NOMANAGER));
    }
}
...
void CUserList::RemoveAllManagers()
{
    ...
    for (int ListPos = 0; ListPos < GetSize(); ListPos++)
	{
        GetAt(ListPos).GetClub().DoManagerJoinedClub(NOMANAGER);
	}
 }
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CUserList::AddToNews(CActiveCountry& _Country, const CExtString &_Message, bool bUserRelated = false)
{
	EXT_ASSERT(_Message.IsEmpty() == false);
	bool Found = false;
	for (int ListPos = 0; ListPos < GetSize(); ListPos++)
	{
		if (GetAt(ListPos).IsActive() && GetAt(ListPos).IsManagingAClubInCountry(_Country))
		{
			GetAt(ListPos).AddToNews(_Message, bUserRelated);
			Found = true;
		}
	}
}
