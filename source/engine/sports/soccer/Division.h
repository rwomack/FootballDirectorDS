...
public:
	...
	const Vector<ClubID> &	GetClubs() const;
	void					AddClub(const ClubID cClubID);
	void					RemoveClub(const ClubID cClubID);
	void 					RemoveAllClubs();
...
#endif
	void					DoAddPlayOffQualifiers(int PlayOffID, CNumericArray<ClubID>& _DrawClubs);
	void					DoAfterMatchUpdate(CFixture& _Fixture);
	void					DoBuildFinishedOrderList(CNumericArray<ClubID>& _ClubsStartingOrder, int _FirstOffset);
	...
	ushort					DoCountTransferListedPlayers();
	...
	ClubID					DoFindClubIDInTablePosition(ushort _ClubToFind);
	ClubID					DoFindClubIDBottomOfTable();
	...
	ClubID					DoPromoteClubInTablePosition(const int _TablePosition);
	void					DoDailyTasks();
	CFixture&				DoInitialiseFixture(const ClubID _HomeClubID, const ClubID _AwayClubID, CMatchInfo _Match);
	...
	CClub& 					GetClub(ushort _ClubInSlot);
	void					ClubMatchListForCurrentMonth(const ClubID iClubID, CPointerArray<CMatchInfo> & MatchesThisMonth);
	CCountry&				GetCountry();
	byte					GetCountryRanking();
	int						CountGoodFoulersForClub(CClub* pClub);
	void					CreateTransferList();
	void					CreateClubsStartingOrder(CNumericArray<ClubID>& _ClubsStartingOrder);
    void					DeleteAll();
	...
	ushort					GetLeagueMatchResult(const byte _MatchDayNumber, const ClubID _HomeClubID, const ClubID _AwayClubID);
	void					GetClubsInRegion(int _RegionToFind, CNumericArray<ClubID>& _TheClubs);
  ...
  bool          IsEndSeasonToday(const int _MatchDayNumber);
	...
	void					SeekClubFixtureForMatchNumber(const ClubID _ClubID, const byte _MatchNumber, CSoccerResult& _SoccerResult);
	...
	ushort					FriendlyMatchType(const int _DayNumber);
	...
	int						FriendlyNumberToMatchNumber(const int _FriendlyNumber);
#endif
#ifdef	QUICKRESULTS
...
public:
    ...
    // Methods
	void					DoAddPlayOffQualifiers(int _PlayOffID, CNumericArray<ClubID>& _DrawClubs);
	void					DoBuildManagerOfMonth();
#ifndef TESTBED
