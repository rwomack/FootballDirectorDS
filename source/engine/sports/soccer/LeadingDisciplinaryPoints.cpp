...
CPlayerClub::CPlayerClub(const ushort _PlayerID, const ClubID _ClubID)
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
ClubID CPlayerClub::GetClubNum() const
{
	return m_ClubID;
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
CLeadingDiscipinaryPoints::CLeadingDiscipinaryPoints(const ushort _PlayerID, const ClubID _ClubID, const byte _YellowCards, const byte _RedCards) 
...
CLeadingDisciplinaryList::~CLeadingDisciplinaryList()
{
	RemoveList();
}
...
bool CLeadingDisciplinaryList::CheckForList(CDivision& _Division)
{
    ...
    for (uint currclub = 0; currclub < _Division.GetNumberOfClubsInDivision(); currclub++)
	{
        ...
        WorldData().GetClubByID(cClubID).GetFirstTeamSquad().AddLeadingDiscipinaryPoints(cClubID, 1, this);
...
{
	ushort MinPoints = 1;
	CClub &cFirstClubInDivision = WorldData().GetClubByID(_Division.GetClubs()[0]);
	if (cFirstClubInDivision.GetTableData().GetNumberGamesPlayed() > 10)
	{
		MinPoints = 6;
	}
    for (uint currclub = 0; currclub < _Division.GetNumberOfClubsInDivision(); currclub++)
	{
		ClubID cClubID = _Division.GetClubs()[currclub];
		WorldData().GetClubByID(cClubID).GetFirstTeamSquad().AddLeadingDiscipinaryPoints(cClubID, MinPoints, this);
	}
	...
	while  (GetSize() > _ListSize)
	{
		...
		while(num < GetSize() && GetSize() > _ListSize)
		{
			...
			{
				DeletedOne = false;
                if (GetAt(num)->GetTotalScored() <= MinPoints)
				{
					DeleteOne(num);
					DeletedOne = true;
				}
