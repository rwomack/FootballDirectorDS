...
#ifdef NDS
IMPLEMENT_FDMEMORY_BINALLOC(CEmployee);
#endif
...
void CEmployee::DoNewContractRequest(CClub& _Club)
{
	...
	if (CPerson::GetContractEndDate().ContractLengthInDays() < 600)
	{
		switch(CPerson::GetContractRenewalStatus())
		{
			case NONEINPROGRESS:
				if ((GetClub().IsUserControlled() == true) && (IsWillingToNegotiateNewContract(true) == true))
				{
          str.Format(IDS_REQUESTSNEWCONTRACT, GetName());
...
{
    CClub* pClub = WorldData().GetClubList().GetEmployeeClub(this);
	if (pClub == NULL)
	{
...
CString CEmployee::szContractResponse(eContractOfferResponse _eResponse, CContract& _ContractOffered, CClub& _Club)
{
	...
	switch(_eResponse)
	{
      ...
      if( !_Club.IsEmployeeAtClub( this ) )
			{
            szStr.Format(IDS_EMP_CONTRACTACCEPTED, GetName(), CNewsItem::PreprocessClubName(&_Club).CStr());
			}
			else
			{
				// Contract renewal
            szStr.Format(IDS_EMP_CONTRACTRENEWALACCEPTED, GetName(), CNewsItem::PreprocessClubName(&_Club).CStr());
			}
			break;
        case MORESIGNON:
			break;
...
void CEmployeeList::Read(CDataFile* _pDataFile)
{
    ...
    for (uint ListPos = 0; ListPos < nCount; ListPos ++)
	{
        ...
        if (ucHasEmployee == 1)
		{
			CEmployee *pNewEmployee = FD_BIN_NEW CEmployee;
...
void CEmployeeList::DoWeeklyTasks(CCountry& _Country, bool _AvailableEmployees /*= false*/)
{
	if (_AvailableEmployees == true)
	{
		...
		if (GetSize() < MINAVAILEMPS)
		{
			...
			{
				Add(FD_BIN_NEW CEmployee(_Country));
			}
  		}
	}
}
