...
#include "..\system\BitmapFormView.h"
...
/*------------------------------------------------------------------------------
  ...
  Returns:    float
	...
	Purpose:
------------------------------------------------------------------------------*/
float CCountry::GetLoanInterestWeeklyMultiplier()
{
  return .3512338f / 100.0f;
}
...
void CCountryList::DoInitialise()
{
	...
	m_pList = FD_ENGINE_NEW CCountry[CExtArray::GetSize()];
...
/*------------------------------------------------------------------------------
	...
	Purpose:	Constructor.
------------------------------------------------------------------------------*/
CCountryList::CCountryList() : m_pList(NULL)
{
}
...
/*------------------------------------------------------------------------------
	...
	Returns:
	Qualifier:
	Purpose:	Used to clear the active country list vector.
------------------------------------------------------------------------------*/
void CCountryList::DeleteActiveCountryList()
{
	for (int i = 0; i < m_ActiveCountryList.GetSize(); i++)
	{
		CActiveCountry *pCountry = m_ActiveCountryList.GetAt(i);
		delete pCountry;
	}
...
void          CCountry::Write(CDataFile* File)
{
  File->Write( &m_FederationRanking, sizeof(m_FederationRanking) );
  File->Write( &m_Ranking, sizeof(m_Ranking) );
  File->Write( &m_FinancialRatioPercentage, sizeof(m_FinancialRatioPercentage) );
  File->Write( &m_ID, sizeof(m_ID) );

//#ifdef COUNTRYNAME
//  m_strName.Write( File );
//#endif
//  m_strNationality.Write( File );

#ifdef USE_CURRENCIES
  File->Write( &m_CurrencyId, sizeof(m_CurrencyId) );
#endif

#ifdef ACTIVEFEDERATIONS
  File->Write( &m_FederationID, sizeof(m_FederationID) );
#endif
}

void          CCountry::Read(CDataFile* File)
{
  File->Read( &m_FederationRanking, sizeof(m_FederationRanking) );
  File->Read( &m_Ranking, sizeof(m_Ranking) );
  File->Read( &m_FinancialRatioPercentage, sizeof(m_FinancialRatioPercentage) );
...
//#endif
//
//  m_strNationality.Read( File );

#ifdef USE_CURRENCIES
  File->Read( &m_CurrencyId, sizeof(m_CurrencyId) );
#endif

#ifdef ACTIVEFEDERATIONS
  File->Read( &m_FederationID, sizeof(m_FederationID) );
#endif
}
...
/*------------------------------------------------------------------------------
  ...
  Parameter:  CDataFile* File
	...
	Purpose:
------------------------------------------------------------------------------*/
void CCountryList::Write(CDataFile* File)
{
  for(int i = 0; i < GetSize(); i++)
  {
    m_pList[i].Write( File );
 }
  //File->Write(m_pList, sizeof(CCountry) * GetSize());
}
...
/*------------------------------------------------------------------------------
  ...
  Parameter:  CDataFile* File
	...
	Purpose:
------------------------------------------------------------------------------*/
void CCountryList::Read(CDataFile* File)
{
  for(int i = 0; i < GetSize(); i++)
  {
    m_pList[i].Read( File );
  ...
  //File->Read(m_pList, sizeof(CCountry) * GetSize());
}
...
CCountry& CCountryList::DoSelectRandomActive()
{
	...
	if (m_ActiveCountryList.GetSize() > 1)
	{
		do
		{
			CountryId = static_cast<ushort>(RandomNumber.IntLessThan(m_ActiveCountryList.GetSize()));
...
{
	m_ActiveCountryList.Add(FD_ENGINE_NEW CActiveCountry(_Resource, _ID));
}
