...
void CFansReaction::DoFansReactionToSale(CPlayer& _Player, const uint _Cost, int _RatingPositionInSquad, int _RatingPositionInPosition)
{
	if (_RatingPositionInSquad < 4 || _RatingPositionInPosition < 2)
	{
		...
		if (_Cost > 0)
		{
			...
			{
        str.Format(IDS_STARSALEMSG1, _Player.GetClub().GetManagerName(), CNewsItem::PreprocessPlayerName(&_Player).CStr());
			}
			//else
			//{
      //  str.Format(IDS_STARSALEMSG2, _Player.GetClub().GetManagerName(), CNewsItem::PreprocessPlayerName(&_Player).CStr());
			//}
		...
		else
		{
			...
			if (RandomNumber.IntLessThan(10) < 5)
			{
        str.Format(IDS_STARLEAVEMSG1, _Player.GetClub().GetManagerName(), CNewsItem::PreprocessPlayerName(&_Player).CStr());
			}
			else
			{
        str.Format(IDS_STARLEAVEMSG2, _Player.GetClub().GetManagerName(), CNewsItem::PreprocessPlayerName(&_Player).CStr());
			}
		}
		_Player.GetClub().AddToClubNews(str);
	}
...
CString CFansReaction::DoFansReactionToSigning(CPlayer& _Player, int _RatingPositionInSquad, int _RatingPositionInPosition)
{
	...
	if (_RatingPositionInSquad < 4 || _RatingPositionInPosition < 2)
	{
		...
		if (RandomNumber.IntLessThan(10) < 5)
		{
      str.Format(IDS_STARSIGNMSG1, _Player.GetClub().GetManagerName(), CNewsItem::PreprocessPlayerName(&_Player).CStr());
		}
		else
		{
      str.Format(IDS_STARSIGNMSG2, _Player.GetClub().GetManagerName(), CNewsItem::PreprocessPlayerName(&_Player).CStr());
		}
		_Player.GetClub().AddToClubNews(str);
	}
	return str;
