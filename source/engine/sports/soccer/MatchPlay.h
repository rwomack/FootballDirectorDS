...
#endif
#ifdef SHOWTERRITORY
  float                  m_Territory[3];                // For Territory tab
#endif
...
public:
	...
	typedef void (*MakeSubstitutionCallbackFunc)(void *pData, const eWhichTeam _Who, ushort _Player1SquadNumber, ushort _Player2SquadNumber);
	typedef void (*SkipToEndOfMatchCommentaryCallbackFunc)(void *pData);
	typedef void (*SkipToEndOfMatchPenaltyCallbackFunc)(void *pData);

	static void SetMakeSubstitutionCallback(MakeSubstitutionCallbackFunc cFunc, void *pData);
	static void SetSkipToEndOfMatchCommentaryCallback(SkipToEndOfMatchCommentaryCallbackFunc cFunc, void *pData);
	static void SetSkipToEndOfMatchPenaltyShootOutCallback(SkipToEndOfMatchPenaltyCallbackFunc cFunc, void *pData);
	...
	void					DoMatch(CFixture* pFixture, const CString &groundName);
	void					DoMatch(CFixture* pFixture, CCup* pCup, CCupRound* pCupRound, const CString &groundName);
	void					DoMatch(CFixture* pFixture, CCup* pCup, CCupRound* pCupRound, CFixture* pLastFixture, const CString &groundName);
	void					DoMatch(CFixture* pFixture, CDivision* pDivision, CCup* pCup, CCupRound* pCupRound, CFixture* pLastFixture, const CString &groundName);
	void					DoGoalScored();
	void					DoOffSide();
    ...
    void                    SetLastEvent(const CString &TheText);
	...
	CFixture&				GetFixture();
	CFixtureMatchEvents &	GetFixtureMatchEvents();
  ...
  void        SetTacticsChanged() { m_TacticsChanged = true; };
	...
	CPenaltyShootOut* GetPenaltyShootOut() { return &m_PenaltyShootOut; };
	CTactic *   GetTactics( eWhichTeam _Who ) { return &m_Tactic[_Who]; };

	ushort GetTacticsId( eWhichTeam _Who ) { return m_CurrentTactic.GetTacticsID( _Who ); };
	...
	int						AwayTerritory();
	int						HomeTerritory();
    float          TotalTerritory() const
	{
		return m_Territory[0] + m_Territory[1] + m_Territory[2];
	}
    int				HomeAttackPossesion()
	{
		return(m_Territory[HOME] / TotalTerritory()) * 100;
	}
    int				MidfieldPossesion()
	{
		return(m_Territory[AWAY] / TotalTerritory()) * 100;
	}
...
private:
	static MakeSubstitutionCallbackFunc ms_pMakeSubstitutionCallback;
	static void *ms_pMakeSubstitutionCallbackData;

	static SkipToEndOfMatchCommentaryCallbackFunc ms_pSkipToEndOfMatchCommentaryCallback;
	static void *ms_pSkipToEndOfMatchCommentaryCallbackData;

	static SkipToEndOfMatchPenaltyCallbackFunc ms_pSkipToEndOfMatchPenaltyCallback;
	static void *ms_pSkipToEndOfMatchPenaltyCallbackData;
};
/////////////////////////////////////////////////////////////////////////////
