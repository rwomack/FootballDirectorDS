...
// PENALTYSHOOTOUT_TEST is used to test penalty shootouts by preventing possible scoring
//#define PENALTYSHOOTOUT_TEST

const int				TIMEBETWEENSUBSTITUTIONS = 8;
const int				TIMEFIRSTSUBSTITUTIONATTEMPT = 60;
const int				CHANCEMAKESUBSTITUTION = 35;
const int				CHANCEACTUALLYINJUREPLAYER = 33;


#ifdef NDS
CMatchPlay::MakeSubstitutionCallbackFunc CMatchPlay::ms_pMakeSubstitutionCallback = NULL;
void *CMatchPlay::ms_pMakeSubstitutionCallbackData = NULL;

CMatchPlay::SkipToEndOfMatchCommentaryCallbackFunc CMatchPlay::ms_pSkipToEndOfMatchCommentaryCallback = NULL;
void *CMatchPlay::ms_pSkipToEndOfMatchCommentaryCallbackData = NULL;

CMatchPlay::SkipToEndOfMatchPenaltyCallbackFunc CMatchPlay::ms_pSkipToEndOfMatchPenaltyCallback = NULL;
void *CMatchPlay::ms_pSkipToEndOfMatchPenaltyCallbackData = NULL;

void CMatchPlay::SetMakeSubstitutionCallback(MakeSubstitutionCallbackFunc cFunc, void *pData)
{
	ms_pMakeSubstitutionCallback = cFunc;
	ms_pMakeSubstitutionCallbackData = pData;
}

void CMatchPlay::SetSkipToEndOfMatchCommentaryCallback(SkipToEndOfMatchCommentaryCallbackFunc cFunc, void *pData)
{
	ms_pSkipToEndOfMatchCommentaryCallback = cFunc;
	ms_pSkipToEndOfMatchCommentaryCallbackData = pData;
}

void CMatchPlay::SetSkipToEndOfMatchPenaltyShootOutCallback(SkipToEndOfMatchPenaltyCallbackFunc cFunc, void *pData)
{
	ms_pSkipToEndOfMatchPenaltyCallback = cFunc;
	ms_pSkipToEndOfMatchPenaltyCallbackData = pData;
}
#endif
...
void CMatchPlay::DoCommenceHalf()
{
	...
	// Prevent injury message from appearing at start of match
	if( m_TimePlayed != 0 )	m_InjuryTime = 1 + RandomNumber.IntLessThan(4);

	m_MatchActionStage = MATCHINPROGRESS;
	m_actionInfo.DoKickOff(GetClub(m_WhoKicksOff));
...
------------------------------------------------------------------------------*/
void CMatchPlay::DoMatch(CFixture* _Fixture, CCup* _Cup, CCupRound* _CupRound, const CString& _StadiumName)
{
...
------------------------------------------------------------------------------*/
void CMatchPlay::DoMatch(CFixture* _Fixture, const CString &_StadiumName)
{
...
------------------------------------------------------------------------------*/
void CMatchPlay::DoMatch(CFixture* _Fixture, CCup* _Cup, CCupRound* _CupRound, CFixture* _LastFixture, const CString& _StadiumName)
{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CMatchPlay::DoMatch(CFixture* _Fixture, CDivision* _Division, CCup* _Cup, CCupRound* _CupRound, CFixture* _LastFixture, const CString& _StadiumName)
{
	EXT_ASSERT_POINTER(_Fixture, CFixture);
    m_pFixture = _Fixture;
...
#else
		// FDL - ELN - After playing the match, show the progress screen again.
		theApp.OnShowSimProgressScreen();
#endif
	}
}
...
eSubstitutionAction CMatchPlay::DoSwapPlayers(const eWhichTeam _Who, const ushort _SquadNumber)
{
	...
	if (_SquadNumber >= PLAYERSINTEAM)
	{
		...
		if (_SquadNumber < PLAYERSINSQUAD)
		{
    ...
    else if (m_SwpSquadPlayer1 != _SquadNumber)
	{
		...
		if (GetFixture().GetMatchSubs().HasPlayerBeenSubstitutedOn(GetClub(_Who).GetFirstTeamSquad().GetSquad(_SquadNumber)) == false)
		{
	...
	else
	{
		if (m_InjuryTime > 0)
		{
          ...
          Commentary.Format(IDS_INJURYTIME, m_InjuryTime);
				...
				{
          Commentary.LoadString(IDS_INJURYTIMESINGLE);
				}
        ...
        afxDump << "FinalPercentageOfPlay " << " Home % of Play " << m_ActualPlayPercent[HOME] << " Away % of Play " << m_ActualPlayPercent[AWAY] << "\n";
					...
					if (IsMatchDrawn() == true)
					{
						if (m_pDivision != null && m_pDivision->GetPointsForWin() == MLSPOINTS)
						{
							...
							{
							  if( m_MatchActionStage != MATCHREQUIREPENALTYSHOOTOUT )
...
#endif
#ifdef NDS
  	// Highlight fixtures rollout menu icon
  	lLogFmt("Match OVER!!!\n");
...
#ifdef NDS
	m_actionInfo.DoScoredPenaltyGoal(*m_pLastShotTaker, GetClubCurrentPosession());
#else
...
void CMatchPlay::DoPenalty()
{
	...
	else
	{
		if (RandomNumber.IntLessThan(4) < 3)
		{
			// Saved
#ifdef NDS
			m_actionInfo.DoPenaltySaved(*m_pLastShotTaker, GetClubCurrentPosession(), m_TeamCurrentPosession);
#else
		...
		else
		{
			// Missed
#ifdef NDS
			m_actionInfo.DoPenaltyMissed(*m_pLastShotTaker, GetClubCurrentPosession(), m_TeamCurrentPosession);
#else
...
#ifdef NDS
	m_actionInfo.DoBookPlayer(GetClubCurrentPosession().GetPlayer(WhichPlayer), GetClubCurrentPosession());
#else
...
void CMatchPlay::DoTrySendPlayerOff()
{
	...
	m_actionInfo.DoSendPlayerOff(GetClubCurrentPosession().GetPlayer(PlayerSquadNumber), GetClubCurrentPosession());
...
void CMatchPlay::DoTryScoreGoal()
{
  ...
  OgStr.LoadString(IDS_OG);
	...
	{
		// Own goal scored
        m_MatchInfoStr.GetGoalInfoStr(m_TeamCurrentPosession).Add(m_TimePlayed, GetClub(m_TeamCurrentPosession).GetPlayer(m_LastShotTakerSquadNum).GetName() + OgStr);
    if( m_pLastShotTaker != NULL )
    {
      // if last shot taker valid
        m_MatchInfoStr.GetOnTargetInfoStr(m_TeamCurrentPosession).Add(m_TimePlayed, m_pLastShotTaker->GetName());
    }
		m_actionInfo.DoOwnGoal( m_TeamCurrentPosession, GetClub(m_TeamCurrentPosession).GetPlayer(m_LastShotTakerSquadNum), GetClub(GetOpponent(m_TeamCurrentPosession)) );
		return;
  }
...
void CMatchPlay::DoNormalGoalScored()
{
	DoGoalScored();
#ifdef NDS
	m_actionInfo.DoNormalGoalScored(GetClubCurrentPosession().GetPlayer(m_LastShotTakerSquadNum),
									GetClubCurrentPosession(), m_TeamCurrentPosession);
#else
...
void CMatchPlay::DoCornerKick()
{
	...
	m_actionInfo.DoCorner(GetFixture().GetClub(m_TeamCurrentPosession));
	...
	byte CornerType = nCornerPercentChanceEvent.NearestAbove(RandomNumber.IntLessThan(100));
        ...
        case CORNERTYPEGOAL:
			...
			if (GetFixture().DoTryScoreGoal(m_TeamCurrentPosession, m_LastShotTakerSquadNum, m_TimePlayed, true) == true)
			{
                ...
                m_actionInfo.DoScoredGoalFromCorner(m_TeamCurrentPosession, IsBottomCorner,
					GetClubCurrentPosession().GetPlayer(m_LastShotTakerSquadNum), GetClubCurrentPosession());
			}
			else
			{
				// Goalkeeper saved
...
void CMatchPlay::DoIndirectFreeKick()
{
	...
	m_actionInfo.DoFreeKick(GetFixture().GetClub(m_TeamCurrentPosession));
#ifdef BALL_PITCH_LOCATION
	WeightedRandomBallYPosition();
...
void CMatchPlay::DoFreeKick()
{
    ...
    byte FreeKickType = nFreeKickPercentChanceEvent.NearestAbove(RandomNumber.IntLessThan(100));
    ...
    switch(FreeKickType)
	{
        ...
        case FREEKICKTYPEGOAL:
			...
			m_actionInfo.DoFreekickGoal(*m_pLastShotTaker, GetClubCurrentPosession(), m_TeamCurrentPosession);
            break;
    }
}
...
void CMatchPlay::DoShotOffTarget()
{
	DoCreateLastShotTaker();
	DoShotMissed();
  m_actionInfo.DoShotOffTarget(GetClubCurrentPosession().GetPlayer(m_LastShotTakerSquadNum), GetClubCurrentPosession());
...
void CMatchPlay::DoShotOnTarget()
{
	...
	DoShotSaved();
#ifdef NDS
	m_actionInfo.DoShotOnTarget(m_TeamCurrentPosession, GetClubCurrentPosession().GetPlayer(m_LastShotTakerSquadNum),
		GetClubCurrentPosession());
#else
...
void CMatchPlay::DoCreateEvents(const eMatchEvent _WhichEvent, const uint _HowMany)
{
    ...
    for (uint LoopCount = 0; LoopCount < _HowMany; LoopCount++)
	{
		...
		if (RandomNumber.IntLessThan(m_FixtureEvents.GetUpperBound()) > m_TimePlayed)
		{
			...
			if (_WhichEvent == HOMEINJUREPLAYERATTEMPT || _WhichEvent == AWAYINJUREPLAYERATTEMPT)
			{
				if (time > 10 && RandomNumber.IntLessThan(100) < CHANCEACTUALLYINJUREPLAYER)
				{
...
void CMatchPlay::DoInjurePlayer()
{
    ...
    else
    {
    	...
    	CString playerNameStr = pPlayerOff->GetName();
    	SetPlayerName(playerNameStr);
        ...
        if (pPlayerOff->InjuryMatchPerformanceEffect() == 100 || (pPlayerOff->InjuryMatchPerformanceEffect() > 40 && GetClubCurrentPosession().IsUserControlled() != true))
		{
			...
			DoTakePlayerOff(PlayerSquadNumber);
			m_actionInfo.DoTakeInjuredPlayerOff(m_TeamCurrentPosession, PlayerSquadNumber, pPlayerOff->InjuryCarriedOff(), *pPlayerOff, GetClubCurrentPosession());
            ...
            if (GetClubCurrentPosession().IsUserControlled() == false || m_SkipToEndOfMatch == true)
			{
				...
				if (GetFixture().GetMatchSubs().AnySubstitutionsLeft(m_TeamCurrentPosession) == true)
				{
...
#ifdef NDS
						m_actionInfo.DoBringSubOnAfterInjury(*pPlayerOff, GetClubCurrentPosession(), *pPlayerOn);
#else
					...
					else
					{
						...
						m_actionInfo.DoNoSubOnAfterInjury(*pPlayerOff, GetClubCurrentPosession(), GetClubCurrentPosession().DoCountPlayersOnPitch());
                        if (PlayerSquadNumber == 0)
						{
			...
			else
			{
				...
				m_actionInfo.DoSubstituteUserInjuredPlayer(*pPlayerOff, GetClubCurrentPosession());
            }
        }
		else
		{
            // Able to continue
			m_actionInfo.DoInjuredPlayerOK(*pPlayerOff, GetClubCurrentPosession(), (PlayerSquadNumber > 0), m_TeamCurrentPosession);
        }
        ReCalculateMatchChancesImmediately();
	}
}
	...
	if (IsMatchInProgress() == true)
	{
...
#ifdef NDS
		if (ms_pSkipToEndOfMatchCommentaryCallback != NULL)
		{
			(*ms_pSkipToEndOfMatchCommentaryCallback)(ms_pSkipToEndOfMatchCommentaryCallbackData);
		}
#endif
		while(DoMinute() == true )
		{
			...
			// Prevent an infinite loop if it goes into penalty shootouts during skip
			if( m_MatchActionStage == MATCHREQUIREPENALTYSHOOTOUT )
			{
			  lLogFmt("** Infinite Loop Prevented\n");
			  ...
			  if (ms_pSkipToEndOfMatchCommentaryCallback != NULL)
        {
          (*ms_pSkipToEndOfMatchCommentaryCallback)(ms_pSkipToEndOfMatchCommentaryCallbackData);
			  ...
			  if (ms_pSkipToEndOfMatchPenaltyCallback != NULL)
        {
          (*ms_pSkipToEndOfMatchPenaltyCallback)(ms_pSkipToEndOfMatchPenaltyCallbackData);
        }
...
eSubstitutionResult CMatchPlay::OnMakeSubstitution(const eWhichTeam _Who, ushort _Player1SquadNumber, ushort _Player2SquadNumber)
{
  ...
  str.Format(IDS_PLAYCOMINGON, m_TimePlayed, GetClub(_Who).GetPlayer(_Player1SquadNumber).GetName(), GetClub(_Who).GetPlayer(_Player2SquadNumber).GetName());
	...
	ushort Player2ListId = GetClub(_Who).GetFirstTeamSquad().GetSquad(_Player2SquadNumber);
	if (GetFixture().GetMatchSubs().HasPlayerBeenSubstitutedOff(Player2ListId) == false)
	{
		...
		if (GetFixture().GetMatchSubs().MakeSubstitution(m_TimePlayed, _Who, GetClub(_Who).GetFirstTeamSquad().GetSquad(_Player2SquadNumber), Player1ListId) == true)
		{
			iRet = SUBSTMADE;
#ifdef NDS
			if (ms_pMakeSubstitutionCallback != NULL)
			{
				ms_pMakeSubstitutionCallback(ms_pMakeSubstitutionCallbackData, _Who, _Player1SquadNumber, _Player2SquadNumber);
			}
#endif
...
void CMatchPlay::DoMakeSubstitution(const eWhichTeam _Who, ushort _Player1SquadNumber, ushort _Player2SquadNumber)
{
	...
	GetClub(_Who).GetFirstTeamSquad().DoSwapPlayers(_Player1SquadNumber, _Player2SquadNumber);
#ifdef NDS
	m_actionInfo.DoSubstituteUserPlayer(GetClub(_Who).GetPlayer(_Player2SquadNumber), GetClub(_Who).GetPlayer(_Player1SquadNumber), GetClub(_Who));
#else
...
#ifdef NDS
/*------------------------------------------------------------------------------
	Method:   	CMatchPlay::GetFixtureMatchEvents
	Access:    	public
	Returns:   	CFixtureMatchEvents&
	Qualifier:
	Purpose:
------------------------------------------------------------------------------*/
CFixtureMatchEvents& CMatchPlay::GetFixtureMatchEvents()
{
	return m_FixtureEvents;
}
#endif
...
/*------------------------------------------------------------------------------
	Method:   	CMatchPlay::HomeTerritory
	Access:    	public
	Returns:   	int
	Qualifier:
	Purpose:
...
int CMatchPlay::HomeTerritory()
{
  ...
  float a = ((m_Territory[2] + (m_Territory[1] / 2)) / TotalTerritory()) * 100;
	return static_cast<int>(a);
}


/*------------------------------------------------------------------------------
	Method:   	CMatchPlay::AwayTerritory
	Access:    	public
	Returns:   	int
	Qualifier:
	Purpose:
