...
/*------------------------------------------------------------------------------
  ...
  Returns:    float
	...
	Purpose:
------------------------------------------------------------------------------*/
float CDivisionStrength::GetTicketAttendanceRatio(const uint _ticketprice)
{
	EXT_ASSERT(_ticketprice < 100);
  return (float)m_TicketAttendanceRatio[_ticketprice >> 1] / 100;
}
...
CDivisionStrengthList::CDivisionStrengthList()
{
	...
	m_pDivisionStrengthList = FD_ENGINE_NEW CDivisionStrength[GetSize()];
