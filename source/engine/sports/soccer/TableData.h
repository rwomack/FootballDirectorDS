...
int StLeagueTableSortFunc(const void *, const void *);
...
public:
	// Constructors
	CLeagueTable();
	CLeagueTable(const ClubID ClubNumber);
	...
	void					SetClubID(ClubID _ClubID);
	void					SetLeaguePosition(const int _Value);

	// Variables
private:
	ClubID					m_ClubID;			// Not currently used in club
...
public:
	...
	void					SetClubIDs(CNumericArray<ClubID>* _ClubIDs);
