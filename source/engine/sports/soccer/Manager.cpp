...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
CManager::CManager(CPlayer* _pPlayer, const uint _Wage, const byte _Contract, const ClubID _ClubID)
{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
CManager::CManager(CEmployee* _pEmployee, const uint _Wage, const byte _Contract, const ClubID _ClubID)
{
	DoResetVariables();
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
CManager::CManager(const uint _Wage, const byte _Contract, const ClubID _ClubID)
{
	DoResetVariables();
	OnInitialise(_Wage, _Contract, _ClubID);
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
CRating& CManager::GetRatingController()
{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CManager::OnInitialise(const uint _Wage, const byte _Contract, const ClubID _ClubID)
{
    CPerson::SetWage(_Wage);
	CPerson::GetContractEndDate().SetEndDate(_Contract);
#ifdef MANAGEREXPENDITURE
  m_WeekExpenditure = CPerson::GetWage() * .8f;
#endif
	// If manager of a club
	if (_ClubID.id != NOCLUB)
	{
		DoNewClub(WorldData().GetClubByID(_ClubID));
	}
}
...
/*------------------------------------------------------------------------------
	...
	Access:    	public
  	Parameter:  const float howmany
	Returns:   	void
	Qualifier:
	Purpose:
------------------------------------------------------------------------------*/
void CManager::DoRaiseRating(const float howmany)
{
...
/*------------------------------------------------------------------------------
	...
	Access:    	public
  	Parameter:  float howmany
	Returns:   	bool
	Qualifier:
	Purpose:
------------------------------------------------------------------------------*/
bool CManager::DoLowerRating(float howmany)
{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
ClubID CManager::GetLastClubID() const
{
...
void CManager::DoLeaveClub()
{
	...
	m_ManHistory[0].SetClubID(ClubID());
    m_ManHistory[1].SetClubDateLeft(WorldData().GetCurrentDate());
	CPerson::GetContractEndDate().ResetDate();
#ifdef DETAILEDMANAGERHISTORY
    m_ClubHistory.DoResetVariables();
#endif
}
...
{
  	return DoLowerRating(max(_PopularityDifference / 5, .3f));
}
...
{
  	float PopularityDifference = max(_PopularityDifference / 5, .3f);
#ifdef DEBUG
//	afxDump << "Raise rating " << aPopularityDifference << "\n";
#endif
	DoRaiseRating(PopularityDifference);
}
...
bool CManager::PlayedCupMatch(CFixture& _Fixture)
{
    ...
    if ( GoalsFor == GoalsAgainst)
	{
		ShouldBeSacked = DoRatingAdjustmentDrawnMatch(RatingAdjustment);
    }
    // Lost  match
    if ( GoalsFor < GoalsAgainst)
	{
		ShouldBeSacked = DoRatingAdjustmentLostMatch(RatingAdjustment);
    }
	return ShouldBeSacked;
 }
...
// 		DEBUG_ONLY(g_DebugStr.Format(_T("%s, %s, Rating : %f"), _NewClub.GetName(), GetName(), GetRatingController()));
...
void CManager::WonMOM(const CString &_Season)
{
	EXT_ASSERT(IsManagingClub() == true);
#ifdef TROPHYHISTORY
#ifdef DETAILEDMANAGERHISTORYONLYFORUSER
	if (IsUser())
	{
#endif
		m_ClubHistory.GetTrophyHistory().Add(FD_ENGINE_NEW CTrophyHistory(GetCurrentClub()->GetDivision().GetName(), GetCurrentClub()->DoFindID(), _Season, 0, MANAGEROFMONTH));
		m_SeasonHistory.GetTrophyHistory().Add(FD_ENGINE_NEW CTrophyHistory(GetCurrentClub()->GetDivision().GetName(), GetCurrentClub()->DoFindID(), _Season, 0, MANAGEROFMONTH));
		m_CareerHistory.GetTrophyHistory().Add(FD_ENGINE_NEW CTrophyHistory(GetCurrentClub()->GetDivision().GetName(), GetCurrentClub()->DoFindID(), _Season, 0, MANAGEROFMONTH));
#ifdef DETAILEDMANAGERHISTORYONLYFORUSER
	}
#endif
#endif
...
void CManager::WonTitle(bool _IsTopDivision, const CString &_SeasonStr)
{
	...
	{
#endif
		m_ClubHistory.GetTrophyHistory().Add(FD_ENGINE_NEW CTrophyHistory(GetCurrentClub()->GetDivision().GetName(), GetCurrentClub()->DoFindID(), _SeasonStr, 0, DIVISIONWINNER));
	    m_CareerHistory.GetTrophyHistory().Add(FD_ENGINE_NEW CTrophyHistory(GetCurrentClub()->GetDivision().GetName(), GetCurrentClub()->DoFindID(), _SeasonStr, 0, DIVISIONWINNER));
#ifdef DETAILEDMANAGERHISTORYONLYFORUSER
	}
#endif
#endif
#ifdef MANAGEREXPENDITURE
  m_BankBalance += (CPerson::GetWage() * 52) *.1f;
#endif
}
...
void CManager::WonCup(CCup* _Cup, const CString &_SeasonStr)
{
	...
	{
#endif
		m_ClubHistory.GetTrophyHistory().Add(FD_ENGINE_NEW CTrophyHistory(_Cup->GetName(), GetCurrentClub()->DoFindID(), _SeasonStr, 0, CUPWINNER));
		m_CareerHistory.GetTrophyHistory().Add(FD_ENGINE_NEW CTrophyHistory(_Cup->GetName(), GetCurrentClub()->DoFindID(), _SeasonStr, 0, CUPWINNER));
#ifdef DETAILEDMANAGERHISTORYONLYFORUSER
	}
#endif
#endif
...
#ifdef MANAGEREXPENDITURE
  m_BankBalance += (CPerson::GetWage() * 52) *.08f;
#endif
}
...
void CManager::RunnerUpInCup(CCup* _Cup, const CString &_SeasonStr)
{
	...
	{
#endif
		m_ClubHistory.GetTrophyHistory().Add(FD_ENGINE_NEW CTrophyHistory(_Cup->GetName(), GetCurrentClub()->DoFindID(), _SeasonStr, 0, CUPRUNNERUP));
		m_CareerHistory.GetTrophyHistory().Add(FD_ENGINE_NEW CTrophyHistory(_Cup->GetName(), GetCurrentClub()->DoFindID(), _SeasonStr, 0, CUPRUNNERUP));
#ifdef DETAILEDMANAGERHISTORYONLYFORUSER
	}
#endif
#endif
...
#ifdef MANAGEREXPENDITURE
  	m_BankBalance += (CPerson::GetWage() * 52) *.04f;
#endif
 }
...
{
	return m_ManHistory[0].GetClubID().id != NOCLUB;
}
...
{
	if (m_ManHistory[0].GetClubID().id == NOCLUB)
	{
		return false;
	}
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
ClubID CManager::GetCurrentClubID() const
{
...
CClub* CManager::GetCurrentClub()
{
	if (IsManagingClub() == true)
	{
		return &WorldData().GetClubByID(GetCurrentClubID());
	}
	else
	{
		return GetLastClub();
...
CClub* CManager::GetLastClub()
{
	if (GetLastClubID().id != NOCLUB)
	{
		return &WorldData().GetClubByID(GetLastClubID());
	}
	else
	{
		return NULL;
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CManager::DoMakeActive(CPlayer* _Player, const uint _Wage, const byte _Contract, const ClubID _ClubID)
{
	DoResetVariables();
    this->DoCopy(_Player);
	OnInitialise(_Wage, _Contract, _ClubID);
...
{
	return !(GetRatingController().IsNull());
}
...
{
 	int WeekExpenditure = CPerson::GetWage() * .8f;
...
void CManagerList::DoInitialise()
{
    ...
    for (uint LoopCount = 0; LoopCount < nHowMany; LoopCount++)
	{
		CManager *pManager = FD_ENGINE_NEW CManager;
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
ushort CManagerList::DoAddNewManager(CPlayer* _Player, const uint _Wage, const byte _Contract, const ClubID _ClubID)
{
	CManager* pManager;
	ushort NewManagerID = NOMANAGER;
    for (int ListPos = 0; ListPos < GetSize(); ListPos ++)
	{
		pManager = GetAt(ListPos);
		if (pManager->IsActive() == false)
		{
			NewManagerID = ListPos;
			pManager->DoMakeActive(_Player, _Wage, _Contract, _ClubID);
			break;
		}
	}
	if (NewManagerID == NOMANAGER)
	{
		NewManagerID = WorldData().GetManagerList().Add(new CManager(_Player, _Wage, _Contract, _ClubID));
	}
	EXT_ASSERT(NewManagerID != NOMANAGER);
	return NewManagerID;
}
...
void CManagerList::DoNewSeason()
{
    ...
    for (int ListPos = 0; ListPos < GetSize(); ListPos ++)
	{
		 ...
		 if (pManager->IsActive() == true)
		 {
			...
			{
				WorldData().GetClubByID(pManager->GetCurrentClubID()).DoManagerJoinedClub(NOMANAGER);
...
/*------------------------------------------------------------------------------
	...
	Parameter: 	const int _MinRating
	Parameter: 	const int _MaxRating
	Returns:   	void
	Qualifier:
	Purpose:
------------------------------------------------------------------------------*/
void CManagerList::DoBuildUnemployedList(CManagerList& _TheList, const ushort _Country, const int _MinRating, const int _MaxRating)
{
    CManager* pManager;
	for (int LoopCount = 0; LoopCount < GetSize(); LoopCount ++)
	{
		if (GetAt(LoopCount)->IsActive() == true)
		{
			...
			if (pManager->GetRatingController().GetRating() >= _MinRating && pManager->GetRatingController().GetRating() <= _MaxRating &&
				pManager->IsUser() == false && pManager->GetCurrentClubID().id == NOCLUB /*&& pManager->GetNationalityID() == _Country*/)
			{
...
ushort CManagerList::DoFindID(CManager* _Manager)
{
    ...
    for (int ListPos = 0; ListPos < GetSize(); ListPos ++)
	{
		if (GetAt(ListPos)->IsActive() == true && GetAt(ListPos) == _Manager)
		{
            iRet = ListPos;
            break;
        }
