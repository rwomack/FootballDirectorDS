...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
ClubID CClubFriendlies::GetOpponents(const ushort _Index)
{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CClubFriendlies::CreateFriendlyMatch(const ushort FriendlyNum, const ClubID _OpponentClubID, const eWhichTeam Venue)
{
...
void CClubFriendlies::OnNewSeason()
{
	...
	{
		m_FriendlyOpponentsClubID[LoopCount].id = NOCLUB;
...
{
	EXT_ASSERT(FriendlyNum < MAXFRIENDLIES);
	if (m_FriendlyOpponentsClubID[FriendlyNum].id == NOCLUB)
	{
		return TEXT("");
	}
	else
	{
        return WorldData().GetClubByID(m_FriendlyOpponentsClubID[FriendlyNum]).GetName();
	}
}
...
CString CClubFriendlies::GetStadiumNameStr(const ushort FriendlyNum, CClub* _ThisClub)
{
	...
	else
	{
		...
		{
			return WorldData().GetClubByID(m_FriendlyOpponentsClubID[FriendlyNum]).GetStadium()->GetName();
		}
	}
}
...
bool CClubFriendlies::IsFriendlyMatchToday(const ushort _MatchNumber, CMatchInfo& _Match, CClub* _ThisClub)
{
	...
	if (FriendlyNum == NOFRIENDLY || m_FriendlyOpponentsClubID[FriendlyNum].id == NOCLUB)
	{
		return false;
	}
	...
	if (m_FriendlyVenue[FriendlyNum] == HOME)
	{
        HomeClubID = _ThisClub->DoFindID();
		AwayClubID = m_FriendlyOpponentsClubID[FriendlyNum];
	}
	else {
        AwayClubID = _ThisClub->DoFindID();
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
bool CClubFriendlies::CanPlayFriendly(const ushort MatchNumber, const ClubID _ClubID, CDivision* pDivision)
{
...
ushort CClubFriendlies::CountAvailableFriendlies()
{
	...
	{
		iRet += (m_FriendlyOpponentsClubID[LoopCount].id == NOCLUB);
	}
	return iRet;
}
