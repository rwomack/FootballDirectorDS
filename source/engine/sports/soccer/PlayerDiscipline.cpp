...
void CPlayerDiscipline::AfterMatchInSquadUpdate(CClub& _Club, CPlayer& _Player)
{
    ...
    if (m_MatchesSuspended > 0)
	{
        ...
        if (m_MatchesSuspended == 0)
		{
 			CExtString Str;
			// Club news
      Str.Format(IDS_SUSPENSIONOVER, CNewsItem::PreprocessPlayerName(&_Player).CStr());
...
void CPlayerDiscipline::DoBeenSuspended(const byte _HowMany, CClub& _Club, CPlayer& _Player)
{
	...
	if (_HowMany > 1)
	{
    strClubNews.Format(IDS_SUSPENSIONSTARTED, CNewsItem::PreprocessPlayerName(&_Player).CStr(), m_MatchesSuspended);
    strMediaNews.Format(IDS_SUSPENSIONSTARTED, str, m_MatchesSuspended);
    _Club.AddToNewsList(strMediaNews);		// moved from line 174
	}
	else
	{
    strClubNews.Format(IDS_SUSPENDED, CNewsItem::PreprocessPlayerName(&_Player).CStr());
		// Dont swamp the news with trivial suspensions
//    strMediaNews.Format(IDS_SUSPENDED, str);
	}
	_Club.AddToClubNews(strClubNews);
	//_Club.AddToNewsList(strMediaNews); // commented out due to strMediaNews being empty
