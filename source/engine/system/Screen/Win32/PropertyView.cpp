...
CPropertyViewPage::CPropertyViewPage (LPCTSTR lpszTemplateName, UINT nIDCaption /* = 0 */)
	: baseCPropertyViewPage (lpszTemplateName, null)
{
  FDASSERT(AfxIsValidString(lpszTemplateName));
	CommonConstruct(lpszTemplateName, nIDCaption);
}

CPropertyViewPage::CPropertyViewPage (UINT nIDTemplate, UINT nIDCaption /* = 0 */, CWnd* pWnd /*= null*/)
	: baseCPropertyViewPage (nIDTemplate, pWnd)
{
  FDASSERT(nIDTemplate != 0);
...
{
  FDASSERT(m_pView);
  FDASSERT_KINDOF(CPropertyView,m_pView);
	return m_pView;
}
...
int CPropertyViewPage::OnCreate (LPCREATESTRUCT lpCreateStruct)
{
	...
	if ( m_nIDCaption ) {
		CString csBuffer;
    if ( LoadText( csBuffer, m_nIDCaption ) ) {
...
int CPropertyView::GetPageIndex (CPropertyViewPage *pPage) const
{
  FDASSERT(m_hWnd);
  FDASSERT(m_TabCtrl.m_hWnd);
  FDASSERT_KINDOF(CPropertyViewPage, pPage);
...
{
  CString csBuffer; LoadText( csBuffer, nID );
...
BOOL CPropertyView::AddPage (CPropertyViewPage *pPage, LPCTSTR pszTitle, int nImage)
{
  ...
  FDASSERT(pPage);
  FDASSERT_KINDOF(CPropertyViewPage, pPage);
  FDASSERT(!pPage->m_hWnd);
  FDASSERT(pPage->m_lpszTemplateName);
