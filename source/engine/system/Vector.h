...
const ushort          c_nElementsIncrement = 10;
...
public:
    ...
    //m_nElementsIncrement = 10;
		m_Elements = FD_ENGINE_NEW Ttype[m_nElementsMax];
  ...
  Vector(int _ElementsInitial)//, int _ElementIncrement)
    ...
    //m_nElementsIncrement = _ElementIncrement;
		m_Elements = FD_ENGINE_NEW Ttype[_ElementsInitial];
		m_nElementsCurrent = 0;
	}
	...
	Vector<Ttype> &operator=(const Vector<Ttype> &rhs)
	{
		RemoveAll();
		Append(rhs);
	...
	/*------------------------------------------------------------------------------
		...
		Access:    	public
		Parameter: 	const Vector<Ttype> & _Array2
		Returns:   	void
		Qualifier:
		Purpose:
	------------------------------------------------------------------------------*/
	...
	void Add(const Ttype& _Element)
	{
		...
		{
      		int nElementsMaxNew = m_nElementsMax + c_nElementsIncrement;
			Ttype* pElements = FD_ENGINE_NEW Ttype[nElementsMaxNew];
			...
			for (LoopCount = 0; LoopCount < m_nElementsMax; LoopCount++)
			{
      		...
      		for (LoopCount = 0; LoopCount < c_nElementsIncrement; LoopCount++ )
			{
				pElements[m_nElementsMax + LoopCount] = Ttype(0);
			}
	...
	/*------------------------------------------------------------------------------
    Method:     Vector<Ttype>::RemoveFromStart
		Access:    	public
		Parameter: 	const ushort _HowMany
		Returns:   	void
		Qualifier:
		Purpose:
	------------------------------------------------------------------------------*/
  void RemoveFromStart(const ushort _HowMany)
	{
		for (int LoopCount = 0; LoopCount < _HowMany; LoopCount++)
		{
      RemoveAt(0);
	...
	------------------------------------------------------------------------------*/
	void DoShuffleArray(const ushort _StartIndex = 0, ushort _MaxPos = 0)
	{
		EXT_ASSERT(_StartIndex <= _MaxPos);
		EXT_ASSERT(_MaxPos < m_nElementsCurrent);

		if (_MaxPos == 0)
		{
			_MaxPos = m_nElementsCurrent - 1;
		}
		if ((_MaxPos - _StartIndex) > 1)
		{
			ushort PositionToSwapWith;
			for (int LoopCount = _StartIndex; LoopCount <= _MaxPos; LoopCount++)
			{
				PositionToSwapWith = _StartIndex + RandomNumber.IntLessThan(_MaxPos - _StartIndex);	// Choose a random position
				DoSwap(PositionToSwapWith, LoopCount);					// Swap the value at the current position with the value at the random position
			}
		}
	}
};
