...
// we need to define some stuff.  Otherwise, this is all defined there.
    ...
    // Make sure FDASSERT and verify are defined in an ANSI fashion
    ...
    // Make sure FDASSERT and verify are defined

    #ifndef FDASSERT
			#include <crtdbg.h>
      #define FDASSERT(f) _FDASSERTE((f))
		#endif
		#ifndef VERIFY
			#ifdef _DEBUG
        #define VERIFY(x) FDASSERT((x))
...
public:
	...
	MYTYPE& operator=(CT t)
	{
		...
		MYTYPE& operator=(const _bstr_t& bstr)
		{
			if ( bstr.length() > 0 )
			{
				return this->assign(static_cast<PCMYSTR>(bstr), bstr.length());
			}
			else
			{
				this->erase();
	...
	bool Load(UINT nId, HMODULE hModule=NULL)
	{
		...
		CString strRes;
    bLoaded       = FALSE != LoadText( strRes, nId );
		if ( bLoaded )
			*this			= strRes;
	...
	void FormatMessage(PCMYSTR szFormat, ...)
	{
		...
		PMYSTR szTemp;
		ssfmtmsg(FORMAT_MESSAGE_FROM_STRING|FORMAT_MESSAGE_ALLOCATE_BUFFER,
					   szFormat, 0, 0,
	...
	{
		MYTYPE sFormat;
    VERIFY(LoadText( sFormat, nFormatId ) != 0);
		va_list argList;
		va_start(argList, nFormatId);
		PMYSTR szTemp;
		ssfmtmsg(FORMAT_MESSAGE_FROM_STRING|FORMAT_MESSAGE_ALLOCATE_BUFFER,
					   sFormat, 0, 0,
					   reinterpret_cast<PMYSTR>(&szTemp), 0, &argList);
		*this = szTemp;
		LocalFree(szTemp);
		va_end(argList);
	}
	...
	{
    FDASSERT(this->size() > static_cast<MYSIZE>(nIndex));
...
//	void			UnlockBuffer(); { }	// why have UnlockBuffer w/o LockBuffer?
    ...
    FDASSERT(this->size()*sizeof(CT) < 0xffffffffUL - sizeof(SSSHDR));
	...
	{
    FDASSERT(size()*sizeof(CT) < 0xffffffffUL - sizeof(SSSHDR));
		HRESULT hr		= E_FAIL;
    FDASSERT(pStream != 0);
		SSSHDR hdr;
