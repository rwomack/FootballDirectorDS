...
public:
	...
	void					DrawBox(CRect& _Rect, const COLORREF _Color = BLACK, const int _Thickness = 1);
	void					DrawSeperator(const int y, const COLORREF color = BLACK);
	void					DrawLine(CPoint& _PointFrom, CPoint& _PointTo, const COLORREF _Color = BLACK, const int _Thickness = 1);
	int						GetBufferWidth();
	int						GetBufferHeight();
	CWnd*					GetParentWnd();
