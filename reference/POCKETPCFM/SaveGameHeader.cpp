...
#ifdef UNICODE
	char* pAnsiString = new char[nUserClubNameSize + 1];
	_pDataFile->Read(pAnsiString, nUserClubNameSize);	
	mbstowcs(junk, pAnsiString, nUserClubNameSize);
#else
...
void CSaveGameHeader::Write(CDataFile* _pDataFile)
{
	...
	char* pAnsiString = new char[nUserClubNameSize + 1];
#ifdef UNICODE
	wcstombs(pAnsiString, m_UserClubName, nUserClubNameSize + 1);
#endif
