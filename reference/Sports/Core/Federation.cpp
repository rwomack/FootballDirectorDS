...
{
	byte bRet = 0;
	ushort FirstClubID = _Country.FirstClubID();
	ushort LastClubID = _Country.LastClubID();
    for (uint ListPos = 0; ListPos < m_CupList.GetSize(); ListPos ++)
	{
        bRet += WorldData().GetFederationCup(m_CupList.GetAt(ListPos)).DoCountCountryEntrants(FirstClubID, LastClubID);
	}
	return 	bRet;
}
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CFederation::DoListCountryEntrants(CActiveCountry& _Country, CNumericArray<ushort>* _TheList)
{
 	ushort FirstClubID = _Country.FirstClubID();
 	ushort LastClubID = _Country.LastClubID();
    for (uint ListPos = 0; ListPos < m_CupList.GetSize(); ListPos ++)
 	{
         WorldData().GetFederationCup(m_CupList.GetAt(ListPos)).DoListCountryEntrants(FirstClubID, LastClubID, _TheList);
 	}
}
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
bool CFederation::HasEnteredFederationCup(ushort _ClubID)
{
	bool bRet = false;
    for (uint ListPos = 0; ListPos < m_CupList.GetSize(); ListPos ++)
	{
...
CFederationList::CFederationList()
{
	...
	m_pFederationList = new CFederation[CExtArray::GetSize()];
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
CFederation& CFederationList::Find(CString _FedName)
{
