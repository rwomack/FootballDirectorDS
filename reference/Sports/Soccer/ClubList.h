...
public:
	...
	void                    CreateFriendlyMatch(const ushort MatchNumber, const ushort HomeClub, const ushort AwayClub);
	...
	void					DoInitialise();
	void					DoSwapClubIDs(const ushort _Club1ID, ushort _Club2ID);
	void					DoSwapClubDivisionPositions(ushort _Club1ID, ushort _Club2ID);
	void					DoWeeklyClearDown();
	void					DoDailyTasks();
	void					DoAddClubsSuitableForPlayer(CPlayer& _Player, CClubList& _SuitableClubs, CClub* _ClubToAvoid = null);
    void                    DumpNames();
	void					EnsureSomeUnManaged(const ushort _NumberClubsToCheck);
    ushort                  Find(CClub* pClub);
    ushort                  Find(CString ClubName);
	CClub&					GetAt(int _Index);
	CClub&					GetByID(const ushort _ClubID);
