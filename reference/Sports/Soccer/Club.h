...
public:
  ...
  // Data Access Methods
	...
	void					SetCurrentTacticsID(const ushort _TacticsID);
	...
	void					DoAccountsPlus(const CAccountsItemList::eAccountItemType aWhich, const uint aHowMuch);
	...
	void                    DoAddTransferIncome(const uint Cost);
	...
	bool                    DoBidForPlayer(ushort _PlayerBiddingFor, ushort _PlayerToBidForClub);
#ifndef TESTBED
	...
	void 					DoCreateBuyPlayerSearch(CPlayerSearch& _PlayerSearch);
	...
	void					DoInitialiseFromResource(const CString& _szName, CBinaryReader& _Resource, CCountry& _Country);
	void                    DoInitialiseTicketPricesAndBonuses();
	...
	void					DoSelectTactics(CClub* apOpponentClub);
    ...
    void                    DoEndOfSeason(const ushort _ClubID);
    ...
    bool                    IsValidUserControlledTeam(const byte aNumSubs);
	...
	void 					TransferListRandomPlayers(const uint _HowMany);
	...
	CTactic&				GetTactics();
	CTactic&				GetTactics(const byte _TacticsID);
    ...
    ushort					TotalYouths();
    bool					TryAcceptTransferBid(CClubTransferOffer& rTheBid);
	...
	void					WonTrophy(CString title, int Id);
	...
	void					SetID(ushort _Value);

	//    CNewsPaperEvents*       NewsPaperEventPtr();

#ifdef TROPHYHISTORY
	void					TrophyWinners(CString title, int Id, bool IsCup);
	void					TrophyRunnersUp(CString title, int Id, bool IsCup);
