...
/*------------------------------------------------------------------------------
	...
	Access:    	public 
	Parameter: 	const byte _LoanCount
	Returns:   	void
	Qualifier: 	
	Purpose:
------------------------------------------------------------------------------*/
...
void CPlayerSquad::DoAddPlayerToSquad(const ushort _nPlayerID)
{
    ...
    for (uint LoopCount = 0; LoopCount < PLAYERSINSQUAD; LoopCount++)
	{
		...
		{
            m_listSquadID[LoopCount] = _nPlayerID;
            break;
		}
    }
}
...
void CPlayerSquad::DoRemoveFromSquad(ushort _PlayerToRemoveSquadNumber)
{
    ...
    if (_PlayerToRemoveSquadNumber < PLAYERSINTEAM)
	{
        ...
        for (uint nSquadNumber = PLAYERSINTEAM; nSquadNumber < PLAYERSINSQUAD; nSquadNumber++)
		{
			if (m_listSquadID[nSquadNumber] != NOPLAYER)
			{
				...
				{
					// replace him
					m_listSquadID[_PlayerToRemoveSquadNumber] = m_listSquadID[nSquadNumber];
    ...
    if (_PlayerToRemoveSquadNumber < PLAYERSINSQUAD - 1)
	{
		...
		{
            m_listSquadID[nSquadNumber] = m_listSquadID[nSquadNumber + 1];
        }
	}
	// Remove the last player from the squad
    m_listSquadID[PLAYERSINSQUAD - 1] = NOPLAYER;
}
...
int CPlayerSquad::CountPlayersScoredAtLeast(const uint _nMin)
{
    ...
    for (uint nSquadNumber = 0; nSquadNumber < PLAYERSINTEAM; nSquadNumber++)
	{
        if (m_listSquadID[nSquadNumber] != NOPLAYER)
		{
			if (GetPlayer(nSquadNumber).GetNumberGoalsThisSeason() >= _nMin)
			{
				iCount++;
			}
		}
...
byte CPlayerSquad::DoCountAvailableNonLoanedPlayers()
{
    ...
    for (uint num = 0; num < PLAYERSINSQUAD; num ++)
	{
        if (m_listSquadID[num] != NOPLAYER)
		{
            if (GetPlayer(num).IsAvailableForMatch() == true && GetPlayer(num).GetWeeksOnLoan() == 0)
			{
                iRet++;
			}
        }
...
/*------------------------------------------------------------------------------
	...
	Purpose:	Add all the players from this club that have more 
				than _MinDiscPoints
------------------------------------------------------------------------------*/
void CPlayerSquad::AddLeadingDiscipinaryPoints(const ushort _CurrentClubID, const int _MinDiscPoints, CLeadingDisciplinaryList* _pList)
{
    for (uint num = 0; num < PLAYERSINSQUAD; num ++)
	{
        if (m_listSquadID[num] != NOPLAYER)
		{
			...
			{
				_pList->Add(new CLeadingDiscipinaryPoints(m_listSquadID[num], _CurrentClubID,
...
/*------------------------------------------------------------------------------
	...
	Purpose:	Add all the players from this club that have scored more 
				than MinGoals
------------------------------------------------------------------------------*/
void CPlayerSquad::AddLeadScorers(const ushort _CurrentClubID, const int _MinGoals, CLeadingGoalScorerList* _pList)
{
	byte LGoals, CGoals;
    for (uint num = 0; num < PLAYERSINSQUAD; num ++)
	{
        if (m_listSquadID[num] != NOPLAYER)
		{
			...
			{
   				LGoals = GetPlayer(num).GetNumberGoalsThisSeason();
   				CGoals = GetPlayer(num).GetCupGoalsScoredThisSeason();

				if (LGoals + CGoals > _MinGoals)
				{
					_pList->Add(new CLeadingGoalScorer(m_listSquadID[num], _CurrentClubID, LGoals, CGoals));
				}
			}
		}
    }
...
ushort CPlayerSquad::DoCountListedPlayers()
{
	...
	for (uint num = 0; num < PLAYERSINSQUAD; num ++)
	{
        if (m_listSquadID[num] != NOPLAYER)
		{
            if (GetPlayer(num).GetTransferStatus().IsTransferListed() || GetPlayer(num).GetContractEndDate().SeasonsRemaining() < 1)
			{
                iRet++;
			}
        }
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CPlayerSquad::DoDeleteAllPlayers(const ushort _nClubID)
{
    for (uint LoopCount = 0; LoopCount < PLAYERSINSQUAD; LoopCount++)
	{
        if (m_listSquadID[LoopCount] != NOPLAYER)
    ...
    for (uint n = 0; n < PLAYERSINSQUAD; n++)
	{
        ...
        if (m_listSquadID[n] != NOPLAYER)
		{
			...
			if (pPlayer->GetContractEndDate().SeasonsRemaining() == 0)
			{
				...
				{
					if (pPlayer->QualifiesForBosman() == true)
					{
						str.Format(IDS_PLAYERNOCONTRACTBOSMAN, pPlayer->GetName(), pPlayer->GetTransferStatus().GetAskingPrice());
					}
					else
					{
						str.Format(IDS_PLAYERNOCONTRACTNOBOSMAN, pPlayer->GetName(), pPlayer->GetTransferStatus().GetAskingPrice());
					}
...
void CPlayerSquad::DoCreateSquad(CClub* _Club)
{
	...
	for (uint LoopCount =0; LoopCount < 22; LoopCount++)
	{
		if (m_listSquadID[LoopCount] == NOPLAYER)
		{
			m_listSquadID[LoopCount] = WorldData().GetPlayerList().AddNew(_Club, Positions[LoopCount]);
		}
	}
}
...
{
	EXT_ASSERT(_ePosn >= GOALKEEPER || _ePosn <= ATTACKER);
	const uint TotalPlayersInPosition[] =
	{
		3,
		9,
		8,
...
void CPlayerSquad::DoTakeAllPlayersOffTransferList()
{
	...
	for (uint LoopCount = 0; LoopCount < PLAYERSINSQUAD; LoopCount++)
	{
		if (m_listSquadID[LoopCount] != NOPLAYER)
		{
			pPlayer = &GetPlayer(LoopCount);
			EXT_ASSERT_POINTER(pPlayer, CPlayer);
			if (pPlayer->GetTransferStatus().IsTransferListed() == true)
			{
...
void CPlayerSquad::PromoteYouth(CContract* _Contract, CYouth* _pYouth)
{
   ...
   m_listSquadID[SquadNumber] = WorldData().GetPlayerList().TransferFromYouth(_pYouth, _Contract);
}
...
ushort CPlayerSquad::DoRankInSquad(CPlayer& _Player)
{
	...
	return ThePlayers.Find(&_Player);
}
...
bool CPlayerSquad::DoGivePlayerFreeTransferToClub(const ePlayerPosition _ePosn, CClub* _pClub)
{
    ...
    for (int num = MINSQUADSIZE + 1; num < PLAYERSINSQUAD; num++)
	{
		if (m_listSquadID[num] != NOPLAYER)
		{
			...
			{
				ContractRequired.SetContractLength(pPlayer->GetSkills().DoGenerateRandomContractLength(pPlayer->GetAge()));
				ContractRequired.SetWage(pPlayer->GetWage());
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CPlayerSquad::GetAllTransferListed(const ushort _ClubID, CPlayerSearch& _PlayerSearch)
{
    for (uint num = 0; num < PLAYERSINSQUAD; num ++)
	{
        if (m_listSquadID[num] != NOPLAYER)
		{
			...
			{
				_PlayerSearch.DoAddToList(m_listSquadID[num], _ClubID, GetPlayer(num).GetValuation(WorldData().GetCurrentUserClub()->GetCountry()));
			}
		}
	}
}
...
------------------------------------------------------------------------------*/
void CPlayerSquad::DoSelectNonUserControlledTeam(const byte _NumSubs, CClub& _Club)
{
...
void CPlayerSquad::DoSelectTeam(const byte _NumSubs, CClub& _Club)
{
	...
	{
		if (WorldData().GetPlayerList()[m_listSquadID[PlayerCount]].IsAvailableForMatch() == false || WorldData().GetPlayerList()[m_listSquadID[PlayerCount]].GetSkills().GetPosition() == GOALKEEPER)
		{
			BestNumber = PlayerCount;
			Count = 0;
			while(WorldData().GetPlayerList()[m_listSquadID[BestNumber]].IsAvailableForMatch() == false || (WorldData().GetPlayerList()[m_listSquadID[BestNumber]].GetSkills().GetPosition() == GOALKEEPER && Count < 10))
			{
				do
				{
	...
	if (pPlayer == null || pPlayer->IsAvailableForMatch() == false || pPlayer->GetSkills().GetPosition() != GOALKEEPER)
	{
		...
		while(Attempts < 10 && (GetPlayer(BestNumber).IsAvailableForMatch() == false || pPlayer->GetSkills().GetPosition() != GOALKEEPER))
		{
			do
			{
				Attempts++;
...
void CPlayerSquad::DoSwapPlayers(const ushort _playernum, const ushort _playernum1)
{
    ...
    m_listSquadID[_playernum] = m_listSquadID[_playernum1];
    m_listSquadID[_playernum1] = tempplayer;
 }
...
{
    EXT_ASSERT(_FirstPlayer != NOPLAYER && _FirstPlayer < PLAYERSINSQUAD);
...
/*------------------------------------------------------------------------------
	...
	Purpose:	Calculate the average skill of all the players in the specified 
				position
------------------------------------------------------------------------------*/
double CPlayerSquad::DoCalculateAveragePositionAge(const ePlayerPosition _ePosn)
{
	int Number = 0;
	double TotalAge = 0;
    for (uint count = 0; count < PLAYERSINSQUAD; count ++)
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
double CPlayerSquad::DoCalculateAverageTeamPlayerAge()
{
	double TotalAge = 0;
    for (uint count = 0; count < PLAYERSINTEAM; count ++)
	{
...
------------------------------------------------------------------------------*/
double CPlayerSquad::DoCalculateAverageSquadPlayerAge()
{
	double TotalAge = 0;
    for (uint count = 0; count < PLAYERSINSQUAD; count ++)
	{
...
CPlayer* CPlayerSquad::DoFindRandomPlayer()
{
    ...
    do
	{
        Player = RandomNumber.IntLessThan(PLAYERSINSQUAD - 1);
...
CPlayer* CPlayerSquad::DoFindRandomSquadPlayer()
{
    ...
    do
	{
        PlayerID = PLAYERSINTEAM + RandomNumber.IntLessThan(PLAYERSINSQUAD - 1 - PLAYERSINTEAM);
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CPlayerSquad::DoEndOfSeason(CClub& _Club, ushort _ClubID)
{
    for (short PlayerListNumber = PLAYERSINSQUAD - 1; PlayerListNumber > -1; PlayerListNumber--)
	{
		if (m_listSquadID[PlayerListNumber] != NOPLAYER)
		{
			...
			{
				GetPlayer(PlayerListNumber).DoRetired(_ClubID);
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
void CPlayerSquad::NewContractRequests()
{
    CPlayer* pPlayer;
	// Check the players
    for(int count = 0; count < PLAYERSINSQUAD; count ++)
	{
        if (m_listSquadID[count] != NOPLAYER)
		{
			...
			{
				pPlayer->NewContractRequest(pPlayer->GetClub());
...
void CPlayerSquad::DoGivePlayersInjury(const ushort _HowMany, const CInjuryType::eInjurySeverity _Severity)
{
	...
	for (int LoopCount = 0; LoopCount < _HowMany; LoopCount++)
	{
		...
		do
		{
			num = RandomNumber.IntLessThan(PLAYERSINSQUAD - 1);
		}
...
void CPlayerSquad::DoDailyTasks(CClub* _Club, const bool _IsWeekEnd, const bool _DoTraining, const uint _TimeTillNextMatch)
{
	...
	if (WorldData().IsFastTest() == false && _TimeTillNextMatch < 10 && DoCountAvailablePlayers() > MINSQUADSIZE + 1)
	{
