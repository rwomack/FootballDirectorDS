...
/*------------------------------------------------------------------------------
	Method:   	CTransferNegotiation::DoCheckForSwapClubs
	Access:    	public 
	...
	Purpose:
------------------------------------------------------------------------------*/
void CTransferNegotiation::DoCheckForSwapClubs(ushort _Club1ID, ushort _Club2ID)
{
	if (m_SellClubID == _Club1ID)
	{
		m_SellClubID = _Club2ID;
	}
	else if (m_SellClubID == _Club2ID)
	{
		m_SellClubID = _Club1ID;
	}
}
...
void CTransferNegotiation::DailyTasks(CClub* _Club)
{
	...
	if (m_DatesToNegotiation == 0)
	{
		...
		CClub& SellClub = WorldData().GetClub(GetSellClubID());
		if (_Club->GetFirstTeamSquad().TotalSquadSize() == PLAYERSINSQUAD)
		{
			str.Format(IDS_NO_ROOM_SQUAD, pPlayer->GetName(), SellClub.GetName());
			UserMessageBox(str);
			return;
		}
		if (_Club->GetCashBalance() < static_cast<int>(m_TheOffer.GetAmountOffered()))
		{
			str.Format(IDS_LACK_OF_FUNDS, pPlayer->GetName(), SellClub.GetName());
			UserMessageBox(str);
			return;
		}
		...
		{
			str.Format(IDS_CANTSPAREPLAYER, SellClub.GetName(), pPlayer->GetName());
			UserMessageBox(str);
			return;
		}

		str.Format(IDS_PROCEED_WITH_NEG, pPlayer->GetName(), SellClub.GetName(),
...
void CTransferNegotiation::DoTransferPlayer()
{
	WorldData().GetCurrentUserClub()->DoBuyTransferPlayer(*WorldData().GetPlayer(GetPlayerID()),
																	  ...
																	  m_PlayerContract,
																	  &WorldData().GetClub(GetSellClubID()),
																	  m_TheOffer.GetSellOnPercentOffered());
	WorldData().GetClub(GetSellClubID()).DoSellPlayer(*WorldData().GetPlayer(GetPlayerID()),
		m_TheOffer.GetAmountOffered(),
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
ushort CTransferNegotiation::GetSellClubID() const
{
	return m_SellClubID;
}
...
void CTransferNegotiationList::Read(CDataFile* _pDataFile)
{
	...
	{
		CTransferNegotiation *pNew = new CTransferNegotiation();
		m_List.Add(pNew);
...
void CTransferNegotiationList::Write(CDataFile* _pDataFile)
{
    ...
    for (uint ListPos = 0; ListPos < HowMany; ListPos++)
	{
		GetAt(HowMany)->Write(_pDataFile);
    }
}
...
{
	m_List.Add(new CTransferNegotiation(rTheBid, DatesToNegotiation));
}
...
/*------------------------------------------------------------------------------
	Method:   	CTransferNegotiationList::CheckForSwapClubs
	Access:    	public 
	...
	Purpose:
------------------------------------------------------------------------------*/
void CTransferNegotiationList::DoCheckForSwapClubs(ushort _Club1ID, ushort _Club2ID)
{
    for (int ListPos = 0; ListPos < m_List.GetSize(); ListPos ++)
	{
         GetAt(ListPos)->DoCheckForSwapClubs(_Club1ID, _Club2ID);
    }
}
...
void CTransferNegotiationList::CheckIfPlayerInTransferNegotiations(ushort _PlayerNum, CClub* _BiddingClub, CClub* _UserClub)
{
    ...
    for (uint ListPos = 0; ListPos < m_List.GetSize(); ListPos ++)
	{
		...
		{
			str.Format(IDS_PLAYER_SIGNED, WorldData().GetPlayer(_PlayerNum)->GetName(), _BiddingClub->GetName());
			UserMessageBox(str);
