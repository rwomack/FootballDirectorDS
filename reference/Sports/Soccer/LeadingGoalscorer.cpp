...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
CLeadingGoalScorer::CLeadingGoalScorer(ushort PNum, ushort CNum, byte LScore, byte CScore) :
...
void CLeadingGoalScorerList::Sort(const int _Left, const int _Right)
{
    ...
    i = _Left;
    j = _Right;
    x = static_cast<CLeadingGoalScorer*> (GetAt((_Left + _Right) / 2))->GetTotalScored();

	do
	{
        while((static_cast<CLeadingGoalScorer*> (GetAt(i))->GetTotalScored() > x) && i < _Right) i++;
        while((static_cast<CLeadingGoalScorer*> (GetAt(j))->GetTotalScored() < x) && j > _Left) j--;
        if(i <= j)
		{
            TempPtr = static_cast<CLeadingGoalScorer*> (GetAt(i));
            SetAt(i, static_cast<CLeadingGoalScorer*> (GetAt(j)));
			SetAt(j, TempPtr);
            i++;
            j--;
		}
...
bool CLeadingGoalScorerList::CheckForList(CDivision& _Division)
{
	...
	{
        _Division.GetClub(currclub).GetFirstTeamSquad().AddLeadScorers(_Division.GetFirstClubNumberInDivision() + currclub, 1, this);
...
void CLeadingGoalScorerList::DoCreateList(CDivision& _Division, const ushort _ListSize)
{
	...
	while  (GetSize() > _ListSize)
	{
		...
		while(num < GetSize())
		{
			...
			{
				DeletedOne = false;
                if (static_cast<CLeadingGoalScorer*> (GetAt(num))->GetTotalScored() <= MinGoals)
				{
					RemoveAt(num);
					DeletedOne = true;
				}
