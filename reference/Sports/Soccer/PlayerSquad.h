...
private:
 ...
 public:
	...
	void					AddLeadingDiscipinaryPoints(const ushort iCurrentClub, const int MinDiscPoints, CLeadingDisciplinaryList* ListPtr);
	void					AddLeadScorers(const ushort iCurrentClub, const int MinGoals, CLeadingGoalScorerList* ListPtr);
	bool					AllowInjuries();
	...
	double					DoCalculateAveragePositionAge(ePlayerPosition Posn);
	double					DoCalculateAverageTeamPlayerAge();
	double					DoCalculateAverageSquadPlayerAge();
	...
	void                    GetAllTransferListed(const ushort ClubID, CPlayerSearch& PlayerSearch);
	...
	void					DoSelectNonUserControlledTeam(const byte _NumSubs, CClub& _Club);
