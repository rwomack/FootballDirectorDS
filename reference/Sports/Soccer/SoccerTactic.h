...
public:
    ...
    byte					GetFamiliarity();
	ushort					GetPlayerPositionX(ushort _PositionNum);
	ushort					GetPlayerPositionY(ushort _PositionNum);
	int						GetEffectiveness();
	void					DoInitialiseFromResource(const int _Title, CBinaryReader& _Resource);
	bool					IsDefender(int _iPlayerNum);
	bool					IsMidfielder(int iPlayerNum);
	bool					IsAttacker(int iPlayerNum);
	void					ReCreate();
	ePlayerPosition			PlayerPosition(ushort _PlayerNum);
...
public:
    ...
    void                    SetTitle(const ushort Which, CString String);
