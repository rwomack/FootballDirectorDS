...
public:
	...
	// ---------------------------------------------------------------------------
	inline void				ReadSize(FILE* File)
	{
		ASSERT(File != NULL);
		fread(&m_iHowMany, sizeof m_iHowMany, 1, File);
	}

	// ---------------------------------------------------------------------------
	inline void				WriteSize(FILE* File)
	{
		ASSERT(File != NULL);
		fwrite(&m_iHowMany, sizeof m_iHowMany, 1, File);
	}
	...
	CString					GetCountStr()
	{
		...
		{
			szRet.LoadString(IDS_NOPREFERENCE);
			return szRet;
		}
		...
		szRet.Format(IDS_CHOICES, nHowManySelected);
		return szRet;
	}
