...
{
    m_JulianDate = _Resource.ReadUInt32();
}
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
int CDate::ConvertToJulian(const uint _Month, const uint _Day, const uint _Year)
{
	...
	m_JulianDate = s_theDate.GetDay() + (153 * s_theDate.GetMonth() - 457) / 5 + 365 * s_theDate.GetYear() + (s_theDate.GetYear() / 4) - (s_theDate.GetYear() / 100) + (s_theDate.GetYear() / 400) + 1721119;
	return m_JulianDate;
}
...
{
	int res = 1 + (((int)(m_JulianDate + 1.5)) % 7);
	return (eDay)res;
}
...
CString CDate::LongDateStr()
{
	...
	CString str;
	str.LoadString(m_iMonth[s_theDate.GetMonth()]);
	CString rStr;
	rStr.Format(_T("%d %s %d"), s_theDate.GetDay(), str, s_theDate.GetYear());
	return rStr;
}
...
CString CDate::MonthYearStr()
{
	...
	CString strPrint;
	strPrint.LoadString(m_iMonth[s_theDate.GetMonth()]);
	CString szYear;
	szYear.Format(_T("%d"), s_theDate.GetYear());
	CString rStr;
