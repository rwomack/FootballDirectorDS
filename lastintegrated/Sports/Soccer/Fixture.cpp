...
void CFixture::DoCalculateTeamStrength(const eWhichTeam _Who, const byte _Time)
{
	...
	CMatchVarsData& MatchVarsData = WorldData().GetMatchHandler().GetMatchVars().GetMatchVars(_Who);
	...
	double dDefstren, dMidstren, dAttstren;
	double EffectiveSkill;
	for (uint m = 1; m < PLAYERSINTEAM; m ++)
	{
		pPlayer = WorldData().GetPlayer(pClub->GetFirstTeamSquad().GetSquad(m));
		EXT_ASSERT_POINTER(pPlayer, CPlayer);
		if (pPlayer->IsPlaying() == true)
		{
			if (pTactics != null)
			{
				...
				dDefstren = (static_cast<double>(pPlayer->DoCalculateEffectiveSkill(SKILL_TACKLING)) / 100 ) *
					pTactics->GetDefenceEffectiveness(m - 1);
				dMidstren = (static_cast<double>(pPlayer->DoCalculateEffectiveSkill(SKILL_PASSING)) / 100 ) *
					pTactics->GetMidfieldEffectiveness(m - 1);
				dAttstren = (static_cast<double>(pPlayer->DoCalculateEffectiveSkill(SKILL_SHOOTING)) / 100 ) *
					pTactics->GetAttackEffectiveness(m - 1);
				...
				MatchVarsData.AddToShootingAndHeadingSkillList( \
					pPlayer->DoCalculateEffectiveSkill(SKILL_SHOOTING) * pTactics->GetShootingEffectiveness(m - 1),
					pPlayer->DoCalculateEffectiveSkill(SKILL_HEADING) * pTactics->GetHeadingEffectiveness(m - 1));
			}
			else
			{
			...
			WorldData().GetMatchHandler().GetMatchVars().GetSquadPerformance(_Who).SetPlayerCurrentSkill(m, dDefstren + dMidstren + dAttstren, _Time);
			Defstren += dDefstren;
			Midstren += dMidstren;
			Attstren += dAttstren;
		...
		else
		{
			...
			MatchVarsData.AddToShootingAndHeadingSkillList(0, 0);
			WorldData().GetMatchHandler().GetMatchVars().GetSquadPerformance(_Who).SetPlayerCurrentSkill(m, 0, _Time);   // Clear performance by minute
		}
	}
	...
	GetFirstTeamSquad(_Who).DoUpdateTeamStrengths(GetGoalKeeperSkill(_Who), \
		GetDefenderSkill(_Who), GetMidfielderSkill(_Who), GetAttackerSkill(_Who));
}
...
void CFixture::DoCalculateDisciplinaryTotals(const eWhichTeam _Who, CMatchVarsData& _MatchVarsData)
{
	...
	_MatchVarsData.SetNumberPlayersToBook(max(0, Aggression.HowManyPlayersToBook() - GetNumberOfPlayersBooked(_Who)));
	_MatchVarsData.SetNumberPlayersToSendOff(max(0, Aggression.HowManyPlayersToSendOff() - GetNumberOfPlayersSentOff(_Who)));
...
{
	CMatchVarsData& pHomeMatchVars = GetMatchVars().GetMatchVars(HOME);
	DoCalculateDisciplinaryTotals(HOME, pHomeMatchVars);
	CMatchVarsData& pAwayMatchVars = GetMatchVars().GetMatchVars(AWAY);
...
CClub& CFixture::GetClub(const eWhichTeam _Who)
{
	if (_Who == HOME)
	{
		return WorldData().GetClub(m_ClubData[HOME].GetClubID());
	}
	else
	{
		return WorldData().GetClub(m_ClubData[AWAY].GetClubID());
	}
}
...
CString CFixture::GetCompetitionTitle()
{
	...
	switch (GetMatchType())
	{
		...
		case CUPMATCHNEUTRALVENUE:
			CompetitionTitle = WorldData().GetCupList().DoFindCompetitionTitleForFixture(this);
			if (CompetitionTitle.IsEmpty() == true)
			{
				CompetitionTitle = WorldData().GetCharityShieldList().DoFindCompetitionTitleForFixture(this);
			}
			if (CompetitionTitle.IsEmpty() == true)
			{
				CompetitionTitle = WorldData().GetFederationCupList().DoFindCompetitionTitleForFixture(this);
			}
			break;
		case FRIENDLYMATCH:
...
{
	return &WorldData().GetClub(GetWinningClubID());
}
...
{
	return &WorldData().GetClub(GetLosingClubID());
}
...
{
	return WorldData().GetClub(GetWinningClubID()).GetUpperCaseName();
}
...
{
	return WorldData().GetClub(GetLosingClubID()).GetUpperCaseName();
}
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
byte CFixture::DoAdjustGoalAttemptsForPaceSupremacy(const eWhichTeam _Who)
{
	byte bRet = ((WorldData().GetMatchHandler().GetMatchVars().GetMatchVars(_Who).GetAttackZonalStats().GetPace() / 3) > ((WorldData().GetMatchHandler().GetMatchVars().GetMatchVars(GetOpponent(_Who)).GetDefenceZonalStats().GetPace() / 4) +  20));
	bRet -= ((WorldData().GetMatchHandler().GetMatchVars().GetMatchVars(_Who).GetAttackZonalStats().GetPace() / 3) < ((WorldData().GetMatchHandler().GetMatchVars().GetMatchVars(GetOpponent(_Who)).GetDefenceZonalStats().GetPace() / 4) +  40));
	return bRet;
}
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
byte CFixture::DoAdjustGoalAttemptsForLongBallPassing(const eWhichTeam _Who, const byte _LongBallPassing)
{
	...
	switch(_LongBallPassing)
	{
		...
		case 1:
			...
			bRet += (GetDefenderSkill(_Who) > GetAttackerSkill(eOpponent)) * 2;
			// Less goal attempts if weaker defence than opponent attack
			bRet -= (GetAttackerSkill(_Who) < GetDefenderSkill(eOpponent)) * 2;
			break;
		case 2:
		case 3:
			// Extra goal attempts if stronger defence than opponent attack
			bRet += (GetDefenderSkill(_Who) > GetAttackerSkill(eOpponent));
			// Less goal attempts if weaker defence than opponent attack
			bRet -= (GetAttackerSkill(_Who) < GetDefenderSkill(eOpponent));
			break;
		case 4:
		case 5:
			break;
		...
		case 7:
			...
			bRet += (GetMidfielderSkill(_Who) > GetMidfielderSkill(eOpponent));
		...
		case 9:
			// Extra goal attempts if stronger midfield than opponent
			bRet += (GetMidfielderSkill(_Who) > GetMidfielderSkill(eOpponent)) * 2;
			// Less goal attempts if weaker midfield than opponent
			bRet -= (GetMidfielderSkill(_Who) < GetMidfielderSkill(eOpponent)) * 2;
			break;
	}
...
void CFixture::DoGenerateFreeKicks(const eWhichTeam _Who)
{
	...
	{
		NumFreeKicks = GetNumberOfPlayersBooked(eOpponent) + RandomNumber.IntLessThan(min(1, GetNumberOfPlayersBooked(eOpponent) * 3 + WorldData().GetMatchHandler().GetMatchVars().GetTeamAggression(eOpponent)));
...
// 	}
	...
	switch(_eDiscType)
	{
		case BOOKPLAYER:
            ...
            DoAddMatchEvent(_When, (eMatchEvent)(HOMEBOOKING + _Who), (CPersonName)*pDiscPlayer);
			Club.GetSeasonStats().DoAddSeasonBookings(1);
			break;

		case SENDPLAYEROFF:
			pDiscPlayer->DoSendPlayerOff(static_cast<eMatchCompetitionType>(GetMatchType()));
            DoAddMatchEvent(_When, (eMatchEvent)(HOMESENDINGOFF + _Who), (CPersonName)*pDiscPlayer);
...
bool CFixture::DoTryScoreOwnGoal(const eWhichTeam _Who, const byte _When, const ushort _ShotTaker)
{
	...
	{
		DoIncrementShotsOnTarget(_Who);
		GetMatchVars().SetLastScorer(&GetClub(GetOpponent(_Who)).GetPlayer(_ShotTaker));
...
void CFixture::DoAddMatchEvent(const byte _Time, const eMatchEvent _eType, CPersonName& _Name)
{
	...
	m_TheEvents.Add(new CMatchEvent(_Time, _eType, _Name));
}
...
void CFixture::DoAfterMatchUpdate(const ushort _HomePrimaryTactics, const ushort _AwayPrimaryTactics)
{
	...
	for (uint LoopCount = 0; LoopCount < PLAYERSINTEAM; LoopCount++)
	{
		...
		if (GetMatchType() == LEAGUEMATCH)
		{
			...
			{
				GetMatchVars().GetDivision()->GetTeamOfTheWeek().GetAt(LoopCount)->AfterMatchCheck(LoopCount, GetHomeClubID());
			}
			if (bAwayAllowTeamOfWeek == true)
			{
				GetMatchVars().GetDivision()->GetTeamOfTheWeek().GetAt(LoopCount)->AfterMatchCheck(LoopCount, GetAwayClubID());
			}
		}
	}
...
void CFixture::DoCalculateMatchEvents(const byte _CurrentTime, CFixtureMatchEvents& _TheEvents)
{
	if (_CurrentTime < (GetMatchLength() - 1))
	{
		...
		DoAdjustAggression(GetClub(HOME), GetMatchVars().GetMatchVars(HOME), GetMatchVars().GetMatchVars(AWAY));
		DoAdjustAggression(GetClub(AWAY), GetMatchVars().GetMatchVars(AWAY), GetMatchVars().GetMatchVars(HOME));
...
{
	if (GetNumberGoals(HOME) + GetNumberGoals(AWAY) < (MAXIMUMGOALSINMATCH - 2) || RandomNumber.IntLessThan(10) < 3)
	{
...
{
	CMatchVarsData&	HomeClubMatchVars = GetMatchVars().GetMatchVars(HOME);
	CMatchVarsData&	AwayClubMatchVars = GetMatchVars().GetMatchVars(AWAY);
    ...
    if (ra == 1 && GetMatchVars().GetTeamMorale(HOME) > 80)
	{
		HomeGoalAttempts++;
	}
	// Chance of a shot less if team morale is low
    if (ra == 3 && GetMatchVars().GetTeamMorale(HOME) < 30)
	{
		HomeGoalAttempts--;
	}
	...
	ra = 1 + RandomNumber.IntLessThan(5);
	...
	// Chance of an extra shot if team morale is high
	if (ra == 1 && GetMatchVars().GetTeamMorale(AWAY) > 80)
	{
		AwayGoalAttempts++;
	}
	// Chance of a shot less if team morale is low
    if (ra == 3 && GetMatchVars().GetTeamMorale(AWAY) < 30)
	{
		AwayGoalAttempts--;
	}
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
const ushort CFixture::DoDefenderManOfMatch(const eWhichTeam _Who)
{
	ushort iRet = NOPLAYER;
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
const ushort CFixture::DoMidfielderManOfMatch(const eWhichTeam _Who)
{
	ushort iRet = NOPLAYER;
	if (RandomNumber.IntLessThan(100) < 75)
	{
...
void CFixture::DoInitialiseMatch(CFixtureMatchEvents& _TheEvents)
{
	...
	GetMatchVars().DoInitialiseClubs(GetSubsSelect());

    switch(GetMatchType())
	{
		case LEAGUEMATCH:
			DoCalculateAttendance(GetMatchVars().GetDivision()->GetMatchType(WorldData().GetCurrentDate().GetMatchDay()), \
					HomeClub.GetTableData().GetLeaguePosition(),
					AwayClub.GetTableData().GetLeaguePosition(),
		...
		case REPLAYMATCH:
            DoCalculateAttendance(5, 1, 1, 30, 30, GetMatchVars().GetMatchVars(HOME).GetClub().GetStadium()->GetCapacity());
            HomeClub.GetSeasonStats().DoAddTotalAttend(GetAttendance());
			break;
...
------------------------------------------------------------------------------*/
void CFixture::DoCalculateAttendance(const byte _WeekNum, const byte _lpos1, const byte _lpos2, const byte _HomeNumInDiv, const byte _AwayNumInDiv, const uint _Capacity)
{
...
CString CFixture::StadiumName()
{
	...
	switch(GetMatchType())
	{
		...
		case CUPMATCHNEUTRALVENUE:
			strName = TEXT("Wembley Stadium");	//EXT_ASSERT(false);
			break;
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
ushort CFixture::GetLosingClubID()
{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
ushort CFixture::GetWinningClubID()
{
	if (GetNumberGoals(HOME) > GetNumberGoals(AWAY))
	{
		return GetHomeClubID();
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
const bool CFixture::IsClubInvolved(const ushort _ClubID)
{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
const sbyte CFixture::GetShotsOffTarget(const eWhichTeam _Who)
{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
byte CFixture::GetMatchLength() const
{
...
void CFixture::Read(CDataFile* _pDataFile)
{
	...
	m_TheEvents.Read(_pDataFile);
	_pDataFile->Read(&m_ClubData[0], sizeof CMatchEventsStats * 2);
}
...
void CFixture::Write(CDataFile* _pDataFile)
{
	...
	m_TheEvents.Write(_pDataFile);
	_pDataFile->Write(&m_ClubData[0], sizeof CMatchEventsStats * 2);
}
...
void CFixtureList::Read(CDataFile* _pDataFile)
{
	...
	{
		SetSize(nHowMany);
		for (LoopCount = 0; LoopCount < nHowMany; LoopCount++)
		{
	...
	if (nHowMany < GetSize())
	{
		for (LoopCount = nHowMany; LoopCount < GetSize(); LoopCount++)
		{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
ushort CFixtureList::FindID(const ushort _HomeClubID, const ushort _AwayClubID)
{
    ushort iRet = wNOTFOUND;
    for (int LoopCount = 0; LoopCount < GetSize(); LoopCount++)
	{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
CFixture* CFixtureList::FindClubFixture(const uint _FixtureNumber, const ushort _ClubID)
{
...
/*------------------------------------------------------------------------------
	...
	Purpose:
------------------------------------------------------------------------------*/
CFixture* CFixtureList::FindFixture(const ushort _HomeClubID, const ushort _AwayClubID)
{
	EXT_ASSERT(GetSize() > 0);
    CFixture* pRet = null;
    for (int LoopCount = 0; LoopCount < GetSize(); LoopCount++)
