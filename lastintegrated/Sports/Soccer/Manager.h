...
public:
	// Constructors
    CManager(const uint _Wage, const byte _Contract, const ushort _ClubID = NOCLUB);
    CManager(CPlayer* _pPlayer, const uint _Wage, const byte _Contract, const ushort _ClubID);
    CManager(CEmployee* _pEmployee, const uint _Wage, const byte _Contract, const ushort _ClubID);
    CManager();
	...
	void					DoMakeActive(CPlayer* _Player, const uint _Wage, const byte _Contract, const ushort _ClubID);
	void					DoRetire();
	void					DoClearMOM();
	void					DoClearTable();
	...
	void					DoNewSeason();
	void					DoRaiseRating(double howmmany);
	...
	void					DoSwapClubIDs(ushort _Club1ID, const ushort _Club2ID);
	CClub*					GetCurrentClub();
	ushort					GetCurrentClubID() const;
	CString&				GetClubName();
	...
	CClub*					GetLastClub();
	void					DoLeaveClub();
	bool					DoLowerRating(double howmmany);
	void					DoResetVariables();
	bool					IsMatchToday(const CCalendar& _CurrentDate, CMatchInfo& _Match);
	void					OnInitialise(const uint _Wage, const byte _Contract, const ushort _ClubID);
...
public:
	...
	ushort					DoAddNewManager(CPlayer* _Player, const uint _Wage, const byte _Contract, const ushort _ClubID);
	void DoBuildUnemployedList(CManagerList& _TheList, const ushort _Country);
